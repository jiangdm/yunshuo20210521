<template>
  <div id="distributor">
    ...
    <div v-show="titleArr[activeName].name == '股东分红'" class="fixed-btn" @click.stop="getQualification()">
      分红资格
    </div>
    <van-dialog
      v-model="fh_dialog"
    ...
    >
      <div class="fh_dialog-style-main" v-if="dividend_text">
        月销售额还差<em>{{ dividend_text.saleroom_short }}</em
        >元<br />
        直推【{{ dividend_text.recommend.level_name }}】人数还差<em>{{ dividend_text.recommend.short }}</em
        >人
        <br />
