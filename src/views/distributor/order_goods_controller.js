...
export default {
  ...
  methods: {
    ...
    getActiveNewTotal(event) {
      ...
      this.timeoutId = setTimeout(function() {
        ...
        if (that.ActiveOldTotal == _inputTotal) {
          ...
          if (that.activeName == 0) {
            ...
            that.changeCount(
              that.goodsCarts[that.ActiveInputId].id,
              that.goodsCartsTotal[that.ActiveInputId].total,
              _changeNum
            );
            return;
          }
      ...
      }, 1200); // 如果还没有执行就又被触发，会根据上面的clearTimeout来清除并重新开始计算
    },
    numberLeft(_id) {
      ...
      this.numberLeft_timeout = setTimeout(function() {
        ...
        if (that.goodsCartsTotal[_id].total <= 0) {
          ...
          that.changeCount(
            that.goodsCarts[_id].id,
            that.goodsCartsTotal[_id].total,
            -that.save_clicknumber
          );
        }
    ...
    submitAction_tc() {
      ...
      $http
        .get(
          ...
          { goods_id: this.goodsInfo.id, total: that.tc_replenish_number, option_id: optionsId },
          ""
        )
        .then(
          function(response) {
