...
export default {
  ...
  activated() {
    ...
    if(this.$route.query.pos){
      let obj=JSON.parse(decodeURIComponent(this.$route.query.pos));
      this.$store.commit("updateLocation", obj);
      this.$store.commit("setLocation", obj);
      window.localStorage.setItem("myLocation", JSON.stringify(obj));
    }
    this.getcategories();
  },
  mounted() {},
    ...
    getData() {
      ...
      if (this.acText === "近期热门") {
        ...
        json = {
          ...
          is_hot: 1,
          page: 1,
          city: this.$store.state.o2oLocation.city
        };
    ...
    search() {
      this.$router.push(
        this.fun.getUrl("searchResult", {
          city: this.$store.state.o2oLocation.city || '全国'
        })
      );
    },
    initTitle() {
