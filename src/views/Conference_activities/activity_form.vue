    ...
    <div id="member">
      ...
      >
        ...
        <div style="width: 90%; margin: 0 auto; padding-bottom: 1.5rem;">
          <van-button size="large" @click.native="submit" type="primary" v-if="status == 0 && type == 'enroll' && payStatus == 0"
          >立即报名
          </van-button
          >
          <!-- <van-button
...
export default {
  ...
  methods: {
    ...
    update() {
      ...
      $http.post("plugin.activity-apply.api.Activity.updateDiyForm", json).then(
        function(response) {
          if (response.result == 1) {
