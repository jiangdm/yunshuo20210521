<template>
  <div id="Activity">
    ...
    <div class="content_box">
      ...
      >
        <i
          class="iconfont icon-active_music_open xuanzhuan"
          @click.stop="play()"
          v-show="!songStop"
        ></i>
        <i
      ...
      <div class="business_box" v-if="(Date.parse(new Date()) / 1000) >= Data.star_time">
        ...
        <div class="user">
          <div
            class="img"
            v-for="(list, index) in browse_member"
            :style="{ zIndex: index, right: `${index * 24 + 5}px` }"
            :key="index"
          >
            ...
            <van-swipe-item v-for="(item, i) in orders" :key="i">
              <div class="header_box">
                <div class="header">
                  <img
                    :src="
                      !fun.isTextEmpty(item.belongs_to_member)
                        ? item.belongs_to_member.avatar_image
                        : ''
                    "
                  />
                </div>
                <div class="content">
                  ...
                  <div class="right">
                    <span class="money"
                      >{{$i18n.t('money')}}{{ item.price }} {{ item.status_name }}</span
                    >
            ...
            <van-swipe-item v-for="(item, i) in commission_amount" :key="i">
              <div class="header_box">
                <div class="header" v-if="item.has_one_member">
                  <img
                    :src="
                      !fun.isTextEmpty(item.has_one_member)
                        ? item.has_one_member.avatar
                        : ''
                    "
                  />
                </div>
      ...
      <div class="store_into">
        ...
        <div class="fixed-box" @click="linkTo('order')">
          ...
          <span class="text orange">{{
            fun.isTextEmpty(Data.button_name) ? "我的订单" : Data.button_name
          }}</span>
        </div>
        <div class="fixed-box" @click="linkTo('member')">
          <img src="../../assets/images/my_member@2x.png" alt="" />
          <span class="text yellow">{{
            fun.isTextEmpty(Data.button_name) ? "会员中心" : Data.button_name
          }}</span>
        </div>
      </div>
