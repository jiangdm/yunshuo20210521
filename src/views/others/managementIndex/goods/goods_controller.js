...
export default {
  ...
  methods: {
    ...
    getData(data) {
      ...
      that.fun.wxShare(
        ...
        {
          title: !that.fun.isTextEmpty(that.goodsInfo.has_one_share) && !that.fun.isTextEmpty(that.goodsInfo.has_one_share.share_title)?that.goodsInfo.has_one_share.share_title:that.goodsInfo.title,
          imgUrl: !that.fun.isTextEmpty(that.goodsInfo.has_one_share) && !that.fun.isTextEmpty(that.goodsInfo.has_one_share.share_thumb)?that.goodsInfo.has_one_share.share_thumb:that.goodsInfo.thumb,
          description: that.goodsInfo.has_one_share.share_desc
        },
        data => {
    ...
    onCloseSkuImages() {
      ...
      // console.log(_lastOption, _activeOption)
      for (let i = 0; i < _lastOption.length; i++) {
        if (_lastOption[i] != _activeOption[i]) {
          let _specs = Number(_activeOption[i]);
          // console.log("变化的规格序号和specs", i, _specs)

          // for (let ind = 0; ind < that.goodsInfo.has_many_specs[i].specitem.length; ind++) {
          //   if (that.goodsInfo.has_many_specs[i].specitem[ind].id == _specs) {
          //     that.goodsInfo.has_many_specs[i].description = that.goodsInfo.has_many_specs[i].specitem[ind]
          //     that.selectSpecs(that.goodsInfo.has_many_specs[i].specitem[ind])
          //   }
          // }
          ...
          for (let ind = 0; ind < that.goodsInfo.has_many_specs.length; ind++) {
            for (let index = 0; index < that.goodsInfo.has_many_specs[ind].specitem.length; index++) {
              if (that.goodsInfo.has_many_specs[ind].specitem[index].id == _specs) {
                // console.log("对应的specitem", that.goodsInfo.has_many_specs[ind].specitem[index].id)
                that.goodsInfo.has_many_specs[ind].description = that.goodsInfo.has_many_specs[ind].specitem[index];
                // that.selectSpecs(that.goodsInfo.has_many_specs[ind].specitem[index])

                that.popPrice = that.goodsInfo.has_many_options[_activeSkuIndex].product_price; //设置价格
                that.popThumb = that.fun.isTextEmpty(that.goodsInfo.has_many_options[_activeSkuIndex].thumb)
                  ? that.goodsInfo.thumb
                  : that.goodsInfo.has_many_options[_activeSkuIndex].thumb; //设置图片
                ...
                optionsMaxCount = that.goodsInfo.has_many_options[_activeSkuIndex].stock; //库存最大数 用于切换更改买家购买数量
              }
            }
          }
        }
    ...
    addCartByO2O() {
      ...
      if (this.fun.isTextEmpty(this.$route.params.store_id)) {
        that.$dialog.alert({ message: "门店参数错误" });
    ...
    buyNowRequest(_goodsId, _optionsId, _total) {
      ...
      } else if (this.goods_type === "yun_sign_goods") {
      ...
      if (specsManage.length == this.goodsInfo.has_many_specs.length) {
        ...
        for (let i = 0; i < this.goodsInfo.has_many_options.length; i++) {
          if (goodsSpecs == this.setGoodsSpecsBySort(this.goodsInfo.has_many_options[i].specs)) {
            ...
            this.popThumb = this.fun.isTextEmpty(this.goodsInfo.has_many_options[i].thumb)
    ...
    appShare(data) {
      var that = this;
      var _title = that.fun.isTextEmpty(that.goodsInfo.has_one_share.share_title)
        ? that.goodsInfo.title
        : that.goodsInfo.has_one_share.share_title;
      var _link = document.location.href + "&share_tag=2";
      _link = that.fun.isMid(_link, data.info.uid);

      var _imgUrl = that.fun.isTextEmpty(that.goodsInfo.has_one_share.share_thumb)
        ? that.goodsInfo.thumb
    ...
    share(data) {
      ...
      wx.ready(function() {
        ...
        if (that.fun.isTextEmpty(that.goodsInfo.has_one_share)) {
          ...
          _title = that.fun.isTextEmpty(that.goodsInfo.has_one_share.share_title)
            ? that.goodsInfo.title
            : that.goodsInfo.has_one_share.share_title;
          _imgUrl = that.fun.isTextEmpty(that.goodsInfo.has_one_share.share_thumb)
            ? that.goodsInfo.thumb
            : that.goodsInfo.has_one_share.share_thumb;
    ...
    postShow() {
      if (!this.fun.isTextEmpty(this.posterImg)) {
        ...
        $http
          .get("goods.goods-poster.new-goods-poster", { id: this.$route.params.id }, "")
          .then(response => {
            toastPoster.clear();
            if (response.result == 1) {
    ...
    showDefaultActivity(val) {
      if (val.first_strip_key == "coupon") {
        ...
        this.firstActivityCon =
          "最高抵扣" + val.love_name + val.deduction_proportion + ",购买赠送" + val.award_proportion + val.love_name;
      } else if (val.first_strip_key == "max_point_deduct" || val.first_strip_key == "point") {
        this.firstActivityBtn = val.point_name;
        this.firstActivityCon =
          "最高抵扣" + val.point_name + val.max_point_deduct + ",购买赠送" + val.point + val.point_name;
        ...
        if (val.first_strip_key == "ed_money" && val.ed_num == 0) {
          ...
          this.firstActivityCon =
            "本商品满{{$i18n.t('money')}}" + val.ed_money + "元包邮" + ",满" + val.ed_num + "件包邮";
        }
        ...
        if (val.commission_show_level == "1") {
          ...
          this.firstActivityCon =
            "一级佣金:" + val.first_commission + "元" + " " + "二级佣金:" + val.second_commission + "元";
    ...
    getJHSupplier() {
      ...
      $http.get("plugin.jd-supply.frontend.goods.option", { goods_id: goods_id }).then(response => {
        if (response.result == 1) {
  ...
  components: { Swipe, SwipeItem, cMyswipe, cComment, cCommentlist, cFreightpricing, cLive, floatOrder, yzGoodsposter },
