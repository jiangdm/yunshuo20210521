          ...
          <div>
            ...
            <div class="side-bar">
              ...
              <div class="like" @click.stop="toggleLike(item)">
                ...
                <span class="likenum">{{ item.link_count }}</span>
              </div>
      ...
      <div class="answer-page">
        ...
        <div class="answerContainer" v-if="subject != null">
          ...
          <div class="answerList">
            <div
              class="answerItem"
              v-for="item in answerfilter"
              :key="item.id"
              @click="reward(item.id)"
            >{{ item.content }}</div>
          </div>
        </div>
      </div>
    ...
    <van-popup v-model="showCommentPopup" position="bottom" round>
      ...
      <ul class="comment-list" @scroll="commentElScrolling">
        ...
        >
          <li
            class="comment-item"
            v-for="(commentItem,itemIndex) in comments"
            :key="commentItem.id"
            @click="displayCommentActionSheet(itemIndex)"
          >
            <comment-item :data="commentItem">
              <ul
                class="comment-sub-list"
                v-if="commentItem.has_many_comment&&commentItem.has_many_comment.length>0"
              >
                <li
                  @click.stop="displayCommentActionSheet(itemIndex,subItemIndex)"
                  v-for="(subCommentItem,subItemIndex) in commentItem.has_many_comment"
                  :key="subCommentItem.id"
                >
                  <comment-item :data="subCommentItem">
                    <div class="comment-like" @click.stop="likeComment(subCommentItem)" slot="right">
                      <img
                        src="https://www.yunzmall.com/min_img/micro_communities/home_praise_cur@2x.png"
                      ...
                      />
                      <img
                        src="https://www.yunzmall.com/min_img/micro_communities/home_praise@2x.png"
                        class="comment-like-icon"
                        width="0.9375rem"
                        height="0.9375rem"
                        v-show="!subCommentItem.is_like"
