<template>
  <div id="circleDetails" v-if="circleInfo">
    <c-title :hide="false" :text="circleInfo.name"></c-title>
    <div class="member" :style="{ 'background-image': 'url(' + bg_img + ')' }">
      <div class="memberUser">
        ...
        <i
          class="iconfont iconSet icon-fontclass-shezhi2"
          v-if="is_master == 1"
          @click="setting(id)"
        ></i>
      </div>
    ...
    <div class="wrapper">
      ...
      <div class="tabs" @click="switchTabs">
        <div
          class="tabItem"
          :class="currentShowTab == 'attention' ? 'active' : ''"
          data-name="attention"
        >关注</div>
        <div
          style="margin-left: 1.59rem;"
          class="tabItem"
          :class="currentShowTab == 'posts' ? 'active' : ''"
          data-name="posts"
        >帖子</div>
        <div
          style="margin-left: 1.59rem;"
          class="tabItem"
          :class="currentShowTab == 'video' ? 'active' : ''"
          data-name="video"
        >视频</div>
      </div>
      <d-list
        ...
        :showStatusText="circleInfo.is_exist||circlePosts.length>0"
      >
        <div class="cards">
          <div
            v-for="(item, itemIndex) in circlePosts"
            :key="item.id"
            @click="toPostDetails(itemIndex)"
          >
            <post-item :data="item" :showCircle="false">
              <div slot="operator" style="display: flex;">
                <circle-button
                  class="btn"
                  @click="attentionAuthor(itemIndex)"
                  :border="!item.is_attention"
                  height="1.625rem"
                  :style="{ padding:'0.2rem 0.2125rem' }"
                >{{ item.is_attention?'已关注':'关注' }}</circle-button>
                <img
    ...
    <div class="sendBtns">
      <div
        ...
        @click="joinCircle"
      >{{ joinQuitCircleText }}</div>
    </div>
    <div class="postBtn">
      <img
        src="https://www.yunzmall.com/min_img/circleDetailPost@3x.png"
        @click="compile(id)"
        v-if="circleInfo.is_master==1||circleInfo.is_posting == 1 && circleInfo.is_exist == 1&&circleInfo.member_review==1"
      />
    </div>
    <van-action-sheet
      :actions="postActionSheet"
      v-model="showPostActionSheet"
      cancel-text="取消"
      close-on-click-action
      @select="selectPostActionSheet"
    ></van-action-sheet>
  </div>
</template>
