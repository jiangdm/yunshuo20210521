...
export default {
  ...
  methods: {
    showCarListM() {
      console.log("11111111111111111111000000000");
    ...
    showGoodPup(_id, status) {
      ...
      $http
        .get("goods.goods.get-goods-page", { id: _id }, ".")
        .then(res => {
          if (res.result == 1) {
    ...
    onSelect(option) {
      if (option.name == "复制接龙") {
        let message = "社区接龙测,立即参与接龙：" + document.location.href;
    ...
    // 检查购物车商品列表是否有配送方式冲突
    checkGoods() {
      $http.get("memberCart.checkout", { cart_ids: this.cart_ids.join(",") }, "操作中").then(
        response => {
          if (response.result == 1) {
