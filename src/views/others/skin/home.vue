  ...
  <div id="skin_home">
    ...
    <div class="main">
      <div class="main-top">
        ...
        <div
          class="uploadMask"
          v-show="treaty_data && (treaty_data.is_open == 1 || treaty_data.is_open == '1') && !checked"
          @click.stop="checkprotocol"
        ></div>
      </div>
      ...
      <van-popup v-model="showUpload" :overlay="false" position="bottom" class="upload-popup" style="height: 100vh;">
        ...
        <div class="upload-popup-text" v-show="img_fize">
          图片大小{{
            Number(img_fize / 1048576).toFixed(2) > 1
              ? Number(img_fize / 1048576).toFixed(2) + 'M'
              : Number(img_fize / 1024).toFixed(2) + 'KB'
          }}
          ，预计时间{{
            parseInt(Number(img_fize / 1048576).toFixed(2) * 2.5) > 5
              ? parseInt(Number(img_fize / 1048576).toFixed(2) * 2.5)
              : 5
          }}s，请稍等...
        </div>
...
export default {
  ...
  methods: {
    ...
    getReport() {
      $http
        .get(
          'plugin.skin-check.api.skin-check.skin-check',
          { img: this.img_url, is_check: this.checked ? 1 : 0 },
          '...'
        )
        .then(
          response => {
