    ...
    <div id="skin_result">
      ...
      <div class="m-result main-bg">
        ...
        <div class="m-result-center">
          <div class="m-result-left">
            <van-circle
              v-model="currentRate"
              :rate="resultData.report.composite_score"
              size="100px"
              stroke-width="100"
              layer-color="#f8f8f8"
              color="#fe5a73"
              :text="resultData.report.composite_score"
            />
      ...
      <div class="main-bg skin-counselor" v-if="!fun.isTextEmpty(resultData.adviser)">
        ...
        <div
          ...
          v-for="item in resultData.adviser"
          :key="item.uid"
          @click.stop="gotoUrl('BusinessCard', {}, { mark: 'card', mark_id: item.business_card.id })"
        >
          ...
          <div class="counselor-list-main">
            <div class="counselor-nickname">
              ...
              <i
                class="iconfont icon-fontclass-mingpian"
                v-if="resultData.business_card && item.business_card && item.business_card.id"
              ></i>
            </div>
            <div class="counselor-name" v-show="!fun.isTextEmpty(item.realname)">姓名：{{ item.realname }}</div>
          </div>
          <div class="counselor-contact">
            <a :href="'tel:' + item.mobile" style="color: #000;" @click.stop
              ><i class="iconfont icon-zx_map_tel" v-show="!fun.isTextEmpty(item.mobile)"></i
            ></a>

            <div class="line" v-show="!fun.isTextEmpty(item.yz_member.wechat)"></div>
            <i
              class="iconfont icon-all_wechat"
              v-show="!fun.isTextEmpty(item.yz_member.wechat)"
      ...
      <div class="skin-goods main-bg">
        ...
        <template v-if="!fun.isTextEmpty(label)">
          <van-tabs v-model="activeLabel" @change="changeLabel">
            <van-tab v-for="item in label" :title="item.label_name" :key="item.id"
              ><em style="color: #fff;">.</em></van-tab
            >
          </van-tabs>
        </template>
        <div
          class="goods-list"
          v-for="item in resultData.goods"
          :key="item.id"
          @click.stop="gotoUrl('goods', { id: item.id }, {})"
        >
          <img :src="item.thumb" alt="" />
          <div class="goods-info">
            <div class="good-title">{{ item.title }}</div>
            <div class="good-effect">
              <div class="good-effect-child" v-for="(child, index) in item.label" :key="index">
                <template v-if="index != 0">|</template>{{ child.label_name }}
              </div>
            </div>
            <div class="goods-price">
              <div class="market_price">
                <template v-if="item.market_price * 100 != 0 && item.market_price != item.price"
                  >{{ fun.getMarketPrice() }}：{{ $i18n.t('money') }}{{ item.market_price }}</template
                >
              </div>
              <div class="prices">
                现价：
          ...
          <div class="diy_poster" id="code_box">
            ...
            >
              ...
              <div class="m-result-right" v-else-if="item.type == 'result'">
                <div
                  class="m-r-r-chilid"
                  v-for="child in item.src"
                  :key="child.id"
                  :style="{ color: item.font_color, fontSize: `${item.size / 16}rem`, backgroundColor: item.color }"
                >
              ...
              <img
                :src="item.src"
                alt=""
                :style="{ width: `${item.width / 16}rem`, height: `${item.height / 16}rem` }"
                v-else
              />
            </div>
          </div>
...
export default {
  ...
  methods: {
    circleM() {
      ...
      context.textAlign = 'center';
      context.textBaseline = 'middle';
      context.fillText('分', centerX + 60, centerY - 10);
      context.restore();
      //绘制蓝色外圈  start
    ...
    changeLabel() {
      ...
      $http
        .get('plugin.skin-check.api.skin-check.label-goods', { label_id: this.label[this.activeLabel].id }, '...')
        .then(
          response => {
