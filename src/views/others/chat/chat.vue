  ...
  <div class="chat-main">
    <div class="chat-scroll" id="chat-data">
      <div class="conversation" v-for="item in chatList" :key="item.id">
        ...
        <template v-else>
          <div class="chat-msg other-msg" v-if="item.direction_type===0">
            <div class="avater">
          ...
          <div class="chat-msg my-msg" v-else-if="item.direction_type==1">
            <div class="avater">
...
export default {
  ...
  methods: {
    init(){
      $http
        .get(
          "plugin.yun-chat.frontend.h5.chat.index",
    ...
    getChatList(){
      ...
      $http
        .get(
          "plugin.yun-chat.frontend.h5.chat.chats",
          { page: 1,employee_id: this.employeeId },
          "加载中"
        )
        .then(response => {
          if (response.result === 1) {
    ...
    sendMsg(content_type){
      ...
      let receiveTextObj = this.generateChatData(text,1,this.getNowDateTime(),content_type);
      let chatListTemp = [];
      ...
      $http
        .post(
          "plugin.yun-chat.frontend.h5.chat.sendMsg",
          {
            text:text,
    ...
    backMsg(item){
      $http
        .get(
          "plugin.yun-chat.frontend.h5.chat.backed-msg",
          { id: item.id},
          "加载中"
        )
        .then(response => {
    ...
    initWebSocket(){ //初始化weosocket
      $http
        .get(
          "plugin.yun-chat.frontend.h5.chat.get-ws-setting"
        )
        .then(response => {
          if (response.result === 1) {
            this.init();
    ...
    websocketonmessage(e){ //数据接收
      ...
      case "init":
        ...
        return;
      case "text":
        if(this.employeeId==message.employee_id) {
          let receiveTextObj = this.generateChatData(message.data,0,message.time,message.content_type);
