<template>
  <div class="chat-main" id="chat-main">
    ...
    <div :style="{'height': currentReconnectNum >= maxReconnectNum ? '80px':'40px'}"></div>
    <div class="no-connect" v-if="currentReconnectNum >= maxReconnectNum">
    ...
    <van-pull-refresh
      v-model="chatLoading"
      @refresh="isDropDownLoad=true;getChatList();"
      pulling-text="释放加载更多聊天信息"
      loosing-text="释放加载更多聊天信息"
      :disabled="chatFinished"
    >
      <div
        class="chat-scroll"
        id="chat-data"
        @click="opetFlag=false"
        :style="{paddingBottom:opetFlag==true ? '17rem' : '3rem'}"
        ref="chatList"
      >
        <div class="conversation" v-for="item in chatList" :key="item.id">
          ...
          <template v-else>
            <div
              class="chat-msg my-msg"
              v-if="(item.direction_type===0 && chatType==2) || (item.direction_type===1 && chatType==1)"
            >
              <div class="avater">
    ...
    <div
      class="chat-footer"
      :style="{bottom: opetFlag==true ? '14rem' : 0}"
    ...
    >
      <div class="chat-opt-input">
        <div class="chat-opt-input-box">
          ...
          <div class="inputWrapper">
            <input id="text_in" v-model="msgText" class="edit-ipt" style @focus="textInputFocus" />
      ...
      <div class="opet">
        <div class="opetItem">
          ...
          >
            <img class="iconImg" src="https://www.yunzmall.com/min_img/chatWindowPic.png" alt />
          </van-uploader>

          <span>图片</span>
        </div>
        <div class="opetItem" @click="showGoodsPopup">
          <img class="iconImg" src="https://www.yunzmall.com/min_img/chatWindowGoods.png" alt />
          <span>商品</span>
        </div>
        <div class="opetItem" @click="showOrderPopup">
          <img class="iconImg" src="https://www.yunzmall.com/min_img/chatWindowOrder.png" alt />
          <span>订单</span>
        </div>
        <div class="opetItem" @click="showReplyPopup" v-if="chatType==2">
          <img class="iconImg" src="https://www.yunzmall.com/min_img/chatWindowQuick.png" alt />
          <span>快捷回复</span>
        </div>
        <div class="opetItem" @click="showTransferPopup" v-if="chatType==2">
          <img class="iconImg" src="https://www.yunzmall.com/min_img/chatWindowTransfer.png" alt />
          <span>转接客服</span>
        </div>
      </div>
      ...
      <div class="search-list-popup-con">
        ...
        <div class="list">
          <div v-for="item in orderList" :key="item.id" class="order-con">
            ...
            <div
              v-for="goods_item in item.has_many_order_goods"
...
export default {
  ...
  methods: {
    ...
    getChatList() {
      ...
      let json = { page: this.chatLoadedPage };
    ...
    sendMsg(content_type) {
      ...
      let json = { text: text, content_type: content_type };
      let url = "";
      let receiveTextObj;
      if (this.chatType == 1) {
      ...
      $http
        ...
        .then(response => {
          if (response.result === 1) {
            ...
            if (addTime) {
              this.chatList.splice(msgIndex, 0, {
                _is_time: true,
    ...
    websocketonmessage(e) {
      ...
      case "text":
        if ((this.chatType == 1 && this.employeeId == message.employee_id) || (this.chatType == 2 && this.userUid == message.uid)) {
          let receiveTextObj;
          if (this.chatType == 1) {
            receiveTextObj = this.generateChatData(message.data, 0, message.time, message.content_type);
          } else if (this.chatType == 2) {
            receiveTextObj = this.generateChatData(message.data, 1, message.time, message.content_type);
          }
