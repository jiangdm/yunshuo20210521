...
export default {
  ...
  methods: {
    ...
    sureIdentify() {
      ...
      $http
        .post("plugin.yun-sign.frontend.h5.company.auth-company-v2", this.form, "...").then(response => {
          if (response.result === 1) {
            // this.show = true;
            // this.checkCompanyData();
            this.$router.push(this.fun.getUrl("identifyResult",{show: 'company'}));
          } else {
            this.$dialog.alert({message:response.msg});
          }
        },
    ...
    checkCompanyData() {
      $http.get("plugin.yun-sign.frontend.company.check-company-approve").then(response => {
        if (response.result === 1) {
