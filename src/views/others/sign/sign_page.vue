<template>
  <div id="contract">
    ...
    <div v-if="showPage">
      <div class="fixed-box" v-if="showMore">
        <div class="triangle"></div>
        <div class="item" @click="toMission">
          <i class="iconfont icon-yq_massage"></i><span>任务信息</span>
        </div>
        <div class="item" @click="getGlobalData">
          <i class="iconfont icon-yq_oline_servce"></i><span>在线客服</span>
        </div>
        <!--<div class="item" @click="downpdf">-->
          <!--<i class="iconfont icon-yq_oline_servce"></i><span>下载合同</span>-->
        <!--</div>-->
      </div>
      <div class="fixed-num" v-if="!signSuccess && !signWait">
      ...
      <div style="transform: translate(0) !important;">
        ...
        <div class="fixed-box1" v-if="(signSuccess || signWait) && showBox">
        ...
        <div class="fixed-img" v-if="signSuccess || signWait" @click="showBox = !showBox">
    ...
    <van-popup v-model="showContent" round position="bottom" :style="{ height: '50%' }">
      ...
      <div class="Identify" v-if="auth_status == 1">
        <div v-if="company_seals.length>0" style="margin-bottom: 0.5rem;">
          <p style="font-size: 15px; font-weight: bold;">企业印章</p>
          <div class="seal-box">
            <div @click.stop="signPic" class="seal-item" v-for="seal in company_seals" :key="seal.id" :id="seal.has_one_seal.id"
                 :style="{backgroundImage: 'url(' + seal.has_one_seal.seal_url + ')'}"></div>
          </div>
        </div>
        <div v-if="person_seals.length>0">
          <p style="font-size: 15px; font-weight: bold;">个人印章</p>
          <div class="seal-box">
            <div @click.stop="signPic" class="seal-item" v-for="seal in person_seals" :key="seal.id" :id="seal.id"
                 :style="{backgroundImage: 'url(' + seal.seal_url + ')'}"></div>
          </div>
        </div>
        <div style="padding-top: 0.5rem; border-top: 1px solid #eee;" v-if="this.commonStructsList.length > 0">
          <van-checkbox v-model="checked">一键落章</van-checkbox>
          <p class="tips">请点击印章，将印章置于合同对应位置！</p>
        </div>
        <div style="padding-top: 0.5rem; border-top: 1px solid #eee;" v-else>
          <p class="tips">请先移动到需要盖章的页数再点击印章！</p>
        </div>
      </div>
    </van-popup>
