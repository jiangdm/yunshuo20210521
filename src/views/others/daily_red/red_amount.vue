<template>
  <div id="RedAmount">
    <c-title :hide="false" :text="'红包额度记录'">
    </c-title>
    ...
    <div class="head">
      ...
      <div class="money">
        <div>
          ...
          <span>{{$i18n.t('money')}}{{receive_logs.amount_sum}}</span>
        </div>
        <div>
          <span class="gray">已使用额度</span>
          <span>{{$i18n.t('money')}}{{receive_logs.receive_amount}}</span>
        </div>
        <div>
          <span class="gray">剩余额度</span>
          <span>{{$i18n.t('money')}}{{(receive_logs.amount_sum - receive_logs.receive_amount).toFixed(2)}}</span>
        </div>
      </div>
    </div>
