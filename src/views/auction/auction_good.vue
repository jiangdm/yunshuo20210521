    ...
    <div v-for="(ithumb, index) in auction.thumb_url"
      ...
      <!--</template>-->
        ...
        <div class="count-time" v-show="now<auction.start_shooting">
          <div class="txt">距离开拍时间</div>
                ...
                <van-count-down :time="fun.getTimeDifference(auction.start_shooting)" class="time" format="DD天HH时mm分ss秒" @finish="timeEnd" v-if="now<auction.start_shooting">
                  <template #default="timeData">
                    <em style="padding: 0.125rem; border-radius: 0.125rem; width: 1.375rem; height: 1.4375rem; background-color: #fff; color: #333;">{{ timeData.days <10 ? `0${timeData.days}` :timeData.days}} </em>天
                    <em style="padding: 0.125rem; border-radius: 0.125rem; width: 1.375rem; height: 1.4375rem; background-color: #fff; color: #333;">{{ timeData.hours <10 ?`0${timeData.hours}` :timeData.hours }}
                    </em>
                    <b>:</b>
                    <em style="padding: 0.125rem; border-radius: 0.125rem; width: 1.375rem; height: 1.4375rem; background-color: #fff; color: #333;">{{ timeData.minutes <10 ?`0${timeData.minutes}` :timeData.minutes }}
                    </em>
                    <b>:</b>
                    <em style="padding: 0.125rem; border-radius: 0.125rem; width: 1.375rem; height: 1.4375rem; background-color: #fff; color: #333;">{{ timeData.seconds <10 ?`0${timeData.seconds}` :timeData.seconds }}
                    </em>
                  </template>
                </van-count-down>
        </div>
        <div class="count-time"  v-show="auction.start_shooting<=now&&now<auction.end_shooting">
          ...
          <div class="txt">距离结束时间</div>
              ...
              <van-count-down :time="fun.getTimeDifference(auction.end_shooting)" class="time" format="DD天HH时mm分ss秒" @finish="timeEnd" v-if="auction.start_shooting<=now&&now<auction.end_shooting">
                  <template #default="timeData">
                    <em style="padding: 0.125rem; border-radius: 0.125rem; width: 1.375rem; height: 1.4375rem; background-color: #fff; color: #333;">{{ timeData.days <10 ? `0${timeData.days}` :timeData.days}} </em>天
                    <em style="padding: 0.125rem; border-radius: 0.125rem; width: 1.375rem; height: 1.4375rem; background-color: #fff; color: #333;">{{ timeData.hours <10 ?`0${timeData.hours}` :timeData.hours }}
                    </em>
          ...
          <div class="count-time"  v-show="now>auction.end_shooting" style="background-color: #999 !important; height: 2.5625rem; display: flex; align-items: center; padding: 0 1rem; background-image: none;">
      ...
      <div class="title">
        <div class="top">
          <div class="left">
            ...
            <del><span style="color: #999; font-size: 12px;">市场价 {{$i18n.t('money')}}{{auction.market_price}}</span></del>
          </div>
          <div>
      ...
      <div class="record-list" v-if="record.length>0">
        <div class="list" v-for="(item,index) in record" :key="index">
            ...
            <div style="font-size: 12px; color: #999; flex: 1;" v-if="item.status==-1">出局</div>
            <div style="font-size: 12px; flex: 1;"><span>{{item.created_at}}</span></div>
            <div style="font-size: 12px; color: #f22121; flex: 0.5;">{{$i18n.t('money')}}{{item.price}}</div>
            <div style="font-size: 12px; flex: 1; text-align: right;" v-if="item.status==1">{{item.number}}次出价</div>
            <div style="font-size: 12px; flex: 1; color: #999; text-align: right;" v-if="item.status==-1">{{item.number}}次出价</div>
        </div>
      ...
      <div class="shop">
        <div class="left">
          ...
          <div class="txt">
            ...
            <div><span class="check" ><i class="iconfont icon-pm_auction_certification" style="color: #ff6333;"></i>{{auctioneer.type_status}}</span><span style="margin-left: 0.5rem; color: #999;">拍品:{{auctioneer.lot_num}}</span></div>
          </div>
        </div>
        <div class="right">
      ...
      <div class="foot" :class="[fun.getPhoneEnv() == 3 ? 'pc' : '']">
        ...
        <div class="puzi" @click="todian" v-else>
            <i
        ...
        <div class="bid-btn" style="background-color: #999 !important;"  v-show="now<auction.start_shooting||now>auction.end_shooting">
      ...
      <div class="bid-btn"  v-show="now<auction.start_shooting||now>auction.end_shooting" style="background-color: #999 !important; margin-left: 1rem;" >
       一口价
      </div>
      </div>
    </div>
...
#auction_good {
  ...
  .content {
    ...
    .count-time {
      ...
      align-items: center;
      padding: 0 1rem;
      background-image: linear-gradient(90deg, #ff830a 0%, #ff3f12 100%);
