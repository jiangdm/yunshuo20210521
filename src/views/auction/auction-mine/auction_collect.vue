      ...
      <div class="list" v-for="(item,index) in datas" :key="index">
         <div class="left">
          ...
          <div class="text">
            ...
            <div style="text-align: left;"><span style="margin-right: 0.5rem; font-size: 12px;" v-if="item.auction_status=='1'"><i class="iconfont icon-pm_auction_certification" style="color: #ff6333;"></i>已认证</span><span style="font-size: 12px; color: #999;">拍品:{{item.lot_num}}</span></div>
          </div>
         </div>
         <div class="cancel" @click="toCollect(item)">取消收藏</div>
      </div>
    </template>
      <!-- <div class="blank"
           v-if="datas.length<=0">
