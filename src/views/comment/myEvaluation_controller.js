  ...
  methods: {
    ...
    opration(btn, item) {
      ...
      if (btn.value == 1) {
        ...
        //CommentDetails
        this.$router.push({ name: "CommentDetails", params: { order_id: item.has_one_comment.order_id, goods_id: item.has_one_comment.goods_id, comment_id: item.has_one_comment.id, uid: 0 }, query: { i: this.toi } });
    ...
    getNetData(status) {
      ...
      $http.get('order.my-comment.paging', json, '获取中').then(response => {
    ...
    getMoreData() {
      if (this.selected == '0') {
        ...
        if (this.waitPage >= this.waitLastPage) {
          ...
          $http.get('order.my-comment.paging', json, '加载中').then((response) => {
