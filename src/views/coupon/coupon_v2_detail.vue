  ...
  <div id="coupon_v2_detail">
    ...
    <div class="centen_box">
      ...
      <div class="couponBtn">
        ...
        <div
          class="cancel"
          v-if="write_off_coupon && info.belongs_to_coupon.plugin_id == '32'"
          @click="tapShow(info.id)"
        >
          核销
        </div>
        <div class="share" v-if="share_coupon" @click="tapShare">分享</div>
      </div>
      <div class="moneyBox">
        <template v-if="coupon_method == 1">
          ...
          <div class="botm_moneyBox" :style="{ top: info.belongs_to_coupon.deduct.length >= 5 ? '4.5rem' : '4rem' }">
            <div :style="{ fontSize: info.belongs_to_coupon.deduct.length >= 5 ? '1.251rem' : '2.251rem' }">
              {{ $i18n.t('money') }}{{ info.belongs_to_coupon.deduct }}
            </div>
          </div>
        </template>
        <template v-if="coupon_method == 2">
          ...
          <span style=" margin: 0 auto; font-size: 2.251rem; margin-top: 1rem; color: #df0132;"
            >{{ info.belongs_to_coupon.discount }}折</span
          >
        </template>
      </div>
      <div class="line"></div>
    ...
    >
      <div class="yz_coupon_dialog">
        ...
        <div class="recharge-num-box" v-show="config_info.switch && nickname != '查无此用户' && recharge_search">
          <div>赠送数量</div>
          <div>
            <van-stepper theme="round" @change="changeRechargeNum" v-model="rechargeNum" min="1" :max="info.num" />
          </div>
        </div>
      </div>
    </van-dialog>
    <van-dialog
      v-model="showRechargePay"
      :title="config_info.protocol_name"
      show-cancel-button
      @confirm="presenterPost"
    >
    ...
    <van-popup v-model="getSucceed" round class="newcomerAward_popup" :style="{ height: '20rem' }">
      <img
        src="https://www.yunzmall.com/min_img/coupons_successbg@2x.png"
        alt=""
        style="width: 15.6rem; height: 18rem;"
      />
    ...
    <van-popup v-model="showSelect" round class="newcomerAward_popup" :style="{ height: '6.906rem' }">
      <div class="shareMum">
        <div class="mumTop">
          分享张数：<van-stepper v-model="shareNum" theme="round" button-size="22" @change="shareMunTap" />
        </div>
        <div class="mumBottom">
...
.moneyBox {
  ...
  height: 9.313rem;
  background-image: url('https://www.yunzmall.com/min_img/coupons_receivebg@2x.png');
