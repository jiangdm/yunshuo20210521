<template>
  <div id="coupon_share_code">
    ...
    <div style="height: 40px; width: 100%; margin-bottom: 1rem;"></div>
          ...
          <div class="shopCoupon">
            <div class="shopCouponNew">
              <div class="left">
                <div
                    class="postLeft"
                    :style="{ 'backgroundImage':(item.combine && item.num != 1 ?`url(https://www.yunzmall.com/min_img/couponsbgreceive_${item.type}@2x.png)`:`url(https://www.yunzmall.com/min_img/couponsbg_${item.type}.png)`) }"
                  >
                    ...
                    <template v-if="item.belongs_to_coupon.coupon_method == 1">
                      <span
                        class="textOne"
                        :style="{'fontSize':(item.belongs_to_coupon.deduct.length > 5?'0.7rem':''),'marginLeft':(item.combine && item.num != 1?'0.6rem':'0')}"
                      >{{ item.belongs_to_coupon.deduct }}</span>
                ...
                <div class="right_bottom">
                  <div class="left_message">
                      ...
                      <div @click="toggle($event, index)">
                        <i
                          class="fa coupon_voucher_more_arr"
                          :class="{ 'fa-angle-down': index == display, 'fa-angle-right': index != display }"
                        ></i>
                      </div>
                    </div>
                </div>
...
.shopCoupon {
  ...
  .left {
    ...
    .postLeft {
      ...
      align-items: center;
      // background-image: url('https://www.yunzmall.com/min_img/couponsbg_1.png');
      .Chapter_number {
        font-size: 0.6rem;
        color: #fff;
        position: absolute;
