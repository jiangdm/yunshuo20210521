...
export default {
  ...
  methods: {
    ...
    saveData(event) {
      ...
      $http
        .post(
          'plugin.business-card.frontend.controllers.card.card.edit',
          this.form
        )
        .then(
          response => {
            if (response.result === 1) {
              ...
              let subStr = new RegExp(
                'business_card/edit_card/' + this.card_id
              );
              let result = window.location.href.replace(
                subStr,
                'business_card/card_center'
    ...
    getData() {
      $http
        .post(
          'plugin.business-card.frontend.controllers.card.card.index',
            ...
            if (response.result === 1) {
              ...
              if (this.fun.getKey('formCard')) {
                ...
                if (window.localStorage.getItem('chooseItem')) {
                  this.chooseItem = JSON.parse(
                    window.localStorage.getItem('chooseItem')
                  );
                }
              } else {
    ...
    onRead(e, flag) {
      ...
      axios
        .post(
          this.fun.getRealUrl(
            'plugin.business-card.frontend.controllers.card.card.upload-local'
          ),
          fd,
          {
    ...
    toChooseGood() {
      ...
      this.$router.push(
        this.fun.getUrl(
          'micro_shop_add_category',
          {},
          { formCard: this.card_id }
        )
      );
    },
    initShare() {
