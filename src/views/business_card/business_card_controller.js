...
export default {
  ...
  methods: {
    ...
    getCardArticle() {
      ...
      $http
        .get(
          "plugin.business-card.frontend.controllers.card.paging.articles",
          { card_id: this.card_id, page: 1 }
        )
        .then(response => {
          if (response.result == 1) {
            this.ArticleIsLoadMore = true;
    ...
    getCardVideo() {
      ...
      $http
        .get(
          "plugin.business-card.frontend.controllers.card.paging.videos",
          { card_id: this.card_id, page: 1 }
        )
        .then(response => {
          if (response.result == 1) {
    ...
    getStoreCommunities() {
      ...
      $http
        .get(
          "plugin.business-card.frontend.controllers.card.paging.tricks",
          { card_id: this.card_id, page: 1 }
        )
        .then(response => {
          if (response.result == 1) {
    ...
    getLiveList(more) {
      ...
      if (more && this.live_page >= this.total_page) {
        ...
        $http.post("plugin.business-card.frontend.controllers.card.paging.rooms", { card_id: this.card_id, page: that.live_page }, "").then(response => {
          if (response.result === 1) {
  ...
  components: { cTitle, cLive, cVideo, cFlow }
};
