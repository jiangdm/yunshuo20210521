  ...
  <div id="microIndex">
    ...
    <div class="milist">
      <ul>
        ...
        <li v-if="functionList.show_video" @click="goToUrl('VideoList')">
          <div class="top">
    ...
    <van-tabs v-model="active" @change="getClassificationBtn" style="background: #f9f5f5;" sticky swipeable>
      <van-tab title="最新">
        <div class="video_list">
          <c-flow :showLoading="showLoading" @loadMore="loadMore" :list="newClassification" v-if="active == '0'"></c-flow>
        </div>
      </van-tab>
      <van-tab title="附近">
        <div class="video_list">
          <c-flow :showLoading="showLoading" @loadMore="loadMore" :list="nearbyClassification" v-if="active == '1'"></c-flow>
        </div>
      </van-tab>
      <template v-for="(item, index) in cate">
        <van-tab :title="item.name" :key="index">
          <div class="cate-box">
            <div class="cate-child-box" :class="[cateMoreData.id == item.id ? 'active-cate-child' : '']" @click.stop="getCateChild(item.id)">全部</div>
            <div
              class="cate-child-box"
              :class="[cateMoreData.id == cateChild.id ? 'active-cate-child' : '']"
              @click.stop="getCateChild(cateChild.id)"
          ...
          <template v-if="cateList[index] && cateList[index].length > 0">
            <div class="video_list">
              <c-flow :showLoading="showLoading" @loadMore="loadMore" :list="cateList[index]" v-if="active == index + 2"></c-flow>
            </div>
          </template>
        </van-tab>
      </template>
    ...
    <yz_home_button :datas="['home']"></yz_home_button>
  </div>
</template>
