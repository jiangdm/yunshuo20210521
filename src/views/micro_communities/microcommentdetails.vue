        ...
        <div class="comment_bottom">
          <ul class="maincomment">
            <li v-for="(item, index) in StickCommentData" :key="index">
              <div class="maincommentleft" @click="replyCommentsBtn(item.id, '', item.uname, item.content, item.is_own)">
                <img v-if="item.has_one_comment_user" :src="`${item.has_one_comment_user.avatar}?${Date.now()}`" alt="" />
              </div>
              <div class="maincommentright">
                <div class="master_content" @click="replyCommentsBtn(item.id, '', item.uname, item.content, item.is_own)">
                  ...
                  <div class="master_content_right">
                    ...
                    <div class="thumb_up" @click.stop="praiseBtn(item.id, '', index, '', item.is_praise)">
                      <div class="thumb_img">
                ...
                <template v-for="(childitem, childindex) in item.child">
                  <div v-if="item.child" :key="childindex" class="deputy_comment" @click="replyCommentsBtn(childitem.major_cm_id, childitem.id, childitem.uname, childitem.content, childitem.is_own)">
                    <div class="deputy_comment_left" v-if="childitem.has_one_comment_user">
...
"
    ...
    <div class="bottom_operation set-pc-styleLeft">
      <div class="content">
        ...
        <div class="appreciate" v-if="reward_button == 0" @click="moneyShow(0)">
        ...
        <div class="comment">
          ...
          <div class="commentnum">{{ detailsData.comments_num }}</div>
        </div>
      </div>
    </div>
    <van-popup v-model="adminOperationshow" position="bottom" :overlay="true">
      <div class="adminOperation">
        ...
        <div class="top_set" v-if="is_own">
          <div class="top_set_left">
            <img src="../../assets/images/micro_communities/detail_revise@2x.png" alt="" />
          </div>
          <div class="top_set_right" @click="reEditBtn">修改</div>
        </div>
