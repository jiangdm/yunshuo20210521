...
export default {
  ...
  methods: {
    ...
    getMoreData_1(_tag) {
      ...
      if (_tag == 1 && that.page >= that.total_page) {
        ...
        $http
          .get(
            "goods.category.get-children-category",
            {
    ...
    numberRight(_id, _stock, good) {
      ...
      let url = "";
      if (good.plugin_id == 40) {
        url = "plugin.lease-toy.api.member-cart.store";
      } else {
        url = "member.member-cart.store";
      }
      $http.get(url, { goods_id: _id, total: 1 }, "").then(
        function(response) {
          if (response.result === 1) {
            that.updateData();
          } else {
    ...
    submitAction() {
      ...
      let url = "";
      if (this.goodsInfo.plugin_id == 40) {
        url = "plugin.lease-toy.api.member-cart.store";
      } else {
        url = "member.member-cart.store";
      }
      $http.get(url, { goods_id: this.goodsInfo.id, total: 1, option_id: optionsId }, "").then(
        function(response) {
          if (response.result === 1) {
            Toast(response.msg);
            that.updateData();
  ...
  activated() {
    if (this.isGetCart && (this.category_template == "01" || this.category_template == "04")) {
      this.$nextTick(() => {
        this.updateData();
      });
    } else {
