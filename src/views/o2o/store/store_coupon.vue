<template>
  <div id="open_member">
    <c-title :hide="false" text='门店优惠券1111'></c-title>
    <div id="content">
        <van-pull-refresh v-model="isLoading" @refresh="loadTop" success-text="刷新成功">
            <div style="height: 40px;"></div>
            <div class="coupon_box" :key='index' v-for="(item,index) in cupconList" @click="getCoupon(item.id,index)">
                <ul class="coupon_a">
                    <li>{{item.name}}</li>
                    <li>满{{item.enough}}{{$i18n.t('元')}}使用</li>
        ...
        <!-- <div class="coupon_box">
            ...
            <div class="coupon_b">
                <p><span>{{$i18n.t('money')}}</span>10</p>
                <button type="button">已领取</button>
            </div>
        </div> -->
    </div>
  </div>
...
<script>
import controller from "./store_coupon_controller";
