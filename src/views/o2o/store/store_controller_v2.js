...
export default {
  ...
  methods: {
    ...
    chooseTab(tab) {
      ...
      } else if (this.tabIndex2 == "门店拼团") {
        this.getStoreGroup();
      }
    },
    getStoreMember() {
      $http
        .get("plugin.store-cashier.frontend.store.get-store-info.get-store-staff", { store_id: this.$route.params.store_id })
        .then(response => {
    ...
    getStoreRooms() {
      $http
        .get("plugin.store-cashier.frontend.store.get-store-info.get-room", { store_id: this.$route.params.store_id, page: 1 })
        .then(response => {
          if (response.result == 1) {
    ...
    getStoreGroup() {
      ...
      $http
        .get("plugin.fight-groups.frontend.store.frontend-fight-groups.index", { store_id: this.$route.params.store_id, page: 1 })
        .then(response => {
          if (response.result == 1) {
    ...
    getStoreCommunities() {
      ...
      $http
        .get("plugin.store-cashier.frontend.store.get-store-info.get-micro-communities", { store_id: this.$route.params.store_id, page: 1 })
        .then(response => {
          if (response.result == 1) {
    ...
    goToAdress() {
      ...
      //   this.goToWXAdress(point, this.store);
      // } else {
      //   window.location.href = `https://uri.amap.com/navigation?to=${point.lng},${point.lat},${this.store.store_name}&mode=car&policy=1&src=mypage&coordinate=gaode&callnative=0`;
      // }
    },
        ...
        .then(
          response => {
            ...
            if (response.result === 1) {
              ...
              if (response.data.is_fight_groups == 1 && this.tabList.indexOf("门店拼团") <= -1) {
                this.tabList.push("门店拼团");
              }
              ...
              if (this.store.dispatch_type.indexOf(3) != -1) {
                if (this.store.store_delivery && this.store.store_delivery.delivery_status == 1) {
                  ...
                  let delivery_area = this.store.store_delivery ? this.store.store_delivery.delivery_area : [];
