<template>
  <div id="all" style="position: relative; height: 100vh;" :class="[this.fun.getPhoneEnv() == 3 ? 'pcStyle' : '']">
    <div class="content" :style="{ backgroundImage: 'url(' + store.store_barnner_thumb + ')' }" v-show="!loadingPopup">
      ...
      <div class="storeInfo_box" ref="storeInfo_box" :class="{ amout: amout }">
        ...
        <div class="reduction-box">
          <div class="reduction-left" v-if="get_store_full_reduction && get_store_full_reduction.length > 0">
            <span v-for="(item, index) in get_store_full_reduction" :key="index" style="margin-left: 0.5rem;">{{ item }}</span>
          </div>
          <div class="reduction-right" v-if="get_store_full_package">{{ get_store_full_package }}</div>
        </div>
      ...
      <div class="menu_evaluate_business">
        ...
        <div class="goods_box" v-show="tabIndex === 0">
          <div class="sort_box" ref="menuWrapper">
            <ul class="box">
              <li v-for="items in category" :class="{ current1: items.id === category_id }" ref="list_li_menulist" :key="items.id">
                <strong class="first" @click.stop="getSecondCategory(items.id, $event)">
          ...
          <div style="float: right; min-height: 90vh; overflow: hidden; width: 78%; background: #fff;">
                ...
                <van-collapse-item name="1">
                  <div slot="title" class="activetitle">
                    ...
                    <dl v-show="activeName != 1">
                      <dt v-for="(o, index) in secondCategory" :key="index" :class="o.id == secondaryId ? 'addClass' : 'removeClass'" @click.stop="getSecondGoods(o.id)">
                        {{ o.name }}
                      </dt>
                    </dl>
                  ...
                  <div style="overflow-y: scroll; max-height: 32vh;">
                    <dl>
                      <dt v-for="(Secondary, index) in secondCategory" :key="index" :class="Secondary.id == secondaryId ? 'addClass1' : 'removeClass1'" @click.stop="getSecondGoods(Secondary.id)">
                        {{ Secondary.name }}
                      </dt>
                    </dl>
                  </div>
              ...
              <div>
                <div class="food-list">
                  <div class="food-list-hook">
                    ...
                    <div class="Goods" v-for="(food, i) in goods" :key="i" @click.stop="goToGoodsO2O(food)">
                      <div class="goodsImg">
                        <img :src="food.thumb" />
                      </div>
                      <div class="goods_right">
                        <ul class="goodsName">
                          ...
                          <div class="show_price_box" v-if="!fun.isTextEmpty(food.goods_show)">
                            <div class="show_price" :class="[i % 2 == 0 ? 'red_white' : 'white_red']" v-for="(item, i) in food.goods_show" :key="i">{{ item.name }}:{{ item.value }}</div>
                          </div>
                          <template v-else>
                        ...
                        <div class="specBtn" v-if="food.has_option === 1 && (!food.vip_level_status || food.vip_level_status.status == 0)">
                        ...
                        <div class="addBtn" v-if="!store.operating_state && showDis && food.has_option !== 1 && (!food.vip_level_status || food.vip_level_status.status == 0)">
          ...
          <div class="evaluate_list">
            <div class="user_box" v-for="(comment, i) in new_comment" :key="i">
              ...
              <div class="user_info">
                ...
                <div class="goodlnfo" v-if="comment.has_one_goods && comment.has_one_goods.id" @click="gotoGoodDetail(comment.has_one_goods.id)">
        ...
        <div class="store_box" v-show="tabIndex === 2">
          <ul class="tab-box">
            <li v-for="(item, index) in tabList" :key="index" :class="tabIndex2 == item ? 'choose' : ''" @click="chooseTab(item)">
              {{ item }}
            </li>
          </ul>
          <div class="detail-box" v-if="tabIndex2 == '店铺详情'">
            ...
            <div class="detail-item">
              ...
              <p v-if="info" v-html="info" id="store-info"></p>
            </div>
          ...
          <div class="group-box" v-if="tabIndex2 == '门店拼团'">
    ...
    <!--</div>-->
    <!--换成组件-->
    <yz_home_button :datas="['home', 'orderlist', 'member']"></yz_home_button>
  </div>
</template>
<script>
...
#all .van-cell--clickable {
  overflow-x: scroll;
}

#all .van-cell__right-icon {
  margin-left: 13rem;
}
