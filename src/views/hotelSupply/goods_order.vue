    ...
    <div id="content">
      ...
      <ul class="info_message">
        <li>
          ...
          <div class="right">{{hotelInfo.name}}</div>
        </li>
        <li>
          <div class="left">酒店地址:</div>
          <div class="right">{{hotelInfo.address}}</div>
        </li>
      </ul>
      <ul class="detail_b">
        <li class="b_room">
      ...
      <ul class="detail_c">
        <li class="c_invoice">
          <span>是否需要发票</span>
          <van-checkbox checked-color="#f15353" shape="square" v-model="isInvoice"
                        @change="invoiceChange"></van-checkbox>
        </li>
        ...
        <li v-if="isInvoice && invoice_type == '1'"><input type="text" placeholder="输入发票抬头" v-model="fp_tt"></li>
        <li v-if="isInvoice && invoice_type == '2'"><input type="text" placeholder="请填写单位名称" v-model="company_name">
        </li>
        <li v-if="isInvoice && invoice_type == '2'"><input type="text" placeholder="请填写纳税人识别向导" v-model="sbxd"></li>
      ...
      <ul class="detail_d">
        ...
        <!-- <li>
            <span>订单总金额</span>
            <span>{{$i18n.t('money')}}{{order_info.total_price}}</span>
        </li>
        <li>
            <span>总优惠</span>
            <span>-{{$i18n.t('money')}}{{order_info.total_discount_price}}</span>
        </li>
        <li>
            <span>总抵扣</span>
            <span>-{{$i18n.t('money')}}{{order_info.total_deduction_price}}</span>
        </li> -->
      </ul>
      ...
      <div class="explain" v-if="false">
        <h1>预定说明</h1>
        <p> 订单确认后不可被取消；赠送双早；入住时需出示身份证；
          如需续住请提前一天预定；退房时可在前台以实际支付金额
          开具发票。</p>
      </div>
      <div class="foot" :style="{width: this.fun.getPhoneEnv() == 3 ? '375px' : ''}">
        <ul class="foot_a">
          <li>合计：<span>{{$i18n.t("money")}}<font>{{order_info.total_price}}</font></span></li>
        </ul>
