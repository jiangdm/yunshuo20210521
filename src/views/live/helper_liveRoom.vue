...
<template>
  <div id="helper_liveRoom">
    <c-title :hide="false"
             text="直播间"></c-title>
    <div class="videos set-pc-style">
      <div id="helper_video"></div>
      <img class="playbtn"
           src="../../assets/images/play1.png"
           alt=""
           v-if="loadedVideoDataOk"
           @click.stop="noAutoPlaying" />
    </div>
            ...
            <li class="good_child"
              ...
              <div class="g_right">
                ...
                <div class="g_bottom">
                  ...
                  <div class="g_btn"
                       v-show="now_goods != item.id"
                       @click="setActiveGood(item.id)">设置当前讲解</div>
                  <div class="g_btn active_goods"
                       v-show="now_goods == item.id"
                       @click="cancelActiveGood(item.id)">
                    取消当前讲解
                  </div>
                </div>
              </div>
        ...
        <van-tab title="聊天">
          <ul class="chat_list"
            ...
            <img src="../../assets/images/blank.png"
                 alt=""
                 style="width: 5rem; margin: 2rem 0;"
                 v-if="fun.isTextEmpty(messageList)" />
          </ul>
        </van-tab>
        <van-tab title="禁言记录">
          <ul class="member_list">
            <li class="member"
                v-for="(item, index) in bannedSaid"
                :key="index">
              <img :src="item.avatar"
                   alt=""
                   class="avator" />
              <div class="nickname">{{ item.nickname }}</div>
              <div class="btn"
                   @click="bannedMember(item.member_id, true)">解禁</div>
            </li>
          </ul>
          <img src="../../assets/images/blank.png"
               alt=""
               style="width: 5rem; margin: 2rem 0;"
               v-if="fun.isTextEmpty(bannedSaid)" />
        </van-tab>
        <van-tab title="踢人记录">
          <ul class="member_list">
            <li class="member"
                v-for="(item, index) in outRoom"
                :key="index">
              <img :src="item.avatar"
                   alt=""
                   class="avator" />
              <div class="nickname">{{ item.nickname }}</div>
              <div class="btn"
                   @click="outRoomMember(item.member_id, true)">恢复</div>
            </li>
          </ul>
          <img src="../../assets/images/blank.png"
               alt=""
               style="width: 5rem; margin: 2rem 0;"
               v-if="fun.isTextEmpty(outRoom)" />
        </van-tab>
      </van-tabs>
    </div>
    <van-popup v-model="showBannedMember"
               round
               position="bottom"
               class="popup_style">
      <img :src="activeMemberInfo.memberAvatar || ''"
           alt=""
           class="popup_img" />
      ...
      <div class="btn_box">
        <div class="btn_child"
             @click="showBannedMember = false">取消</div>
        <div class="btn_child success"
             @click="bannedMember(activeMemberInfo.memberId)">确认</div>
      </div>
    </van-popup>
    <van-popup v-model="showOutRoomMember"
               round
               position="bottom"
               class="popup_style">
      <img :src="activeMemberInfo.memberAvatar"
      ...
      <div class="btn_box">
        <div class="btn_child"
             @click="showOutRoomMember = false">取消</div>
        <div class="btn_child success"
             @click="outRoomMember(activeMemberInfo.memberId)">确认</div>
      </div>
    </van-popup>
  </div>
</template>
...
export default {
  ...
  mounted() {
    let self = this;
    if (!document.getElementById('TcPlayerJs')) {  //避免多次引入
      const script = document.createElement('script');
      script.type = 'text/javascript';
      if(window.location.host.indexOf('localhost')>-1) {
        script.src = this.fun.getSiteRoot() + '/static/app/TcPlayer-2.3.2.js';
      }else {
        script.src = this.fun.getSiteRoot() + '/addons/yun_shop/static/app/TcPlayer-2.3.2.js';
      }
  ...
  methods: {
    ...
    chatEventList() {
      ...
      tim.on(TIM.EVENT.MESSAGE_RECEIVED, function(event) {
        ...
        message.forEach(function(val, index) {
          ...
          } else if (conversationType == TIM.TYPES.CONV_C2C) { console.log('1');
          }
        });
      });
      ...
      $http
        ...
        .then(response => {
          if (response.result === 1) {
            ...
            this.now_goods = response.data.now_goods.goods_id;
            setTimeout(() => {
              that.chatLogin();
            }, 1000);
          } else {
    ...
    setTcPlayer(_type) {
      ...
      player = new TcPlayer('helper_video', {
        ...
        listener: function(msg) {
          ...
          if (msg.type == 'loadedmetadata') { console.log('loadeddata===============');
          }
          if (msg.type == 'load') {
            setTimeout(() => {
          ...
          if (msg.type == 'play') { console.log('loadeddata===============');
          }
          if (msg.type == 'playing') {
          ...
          if (msg.type == 'timeupdate') { console.log('loadeddata===============');
          }
        }
      });
    },
    ...
    getOther(name) {
      ...
      let _url =
        name == 2 ? 'plugin.room.frontend.anchor-aide.chat-ban-list' : 'plugin.room.frontend.anchor-aide.room-ban-list';
      $http
