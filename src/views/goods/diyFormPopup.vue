    ...
    <div id="member">
      ...
      <div class="form" v-if="loading && !status">
        <van-cell-group
          ...
          :key="i"
          :title="
            item.data_type != 'diyduoxingwenben' && item.data_type != 'diyduoxing' && item.data_type != 'diytupian'
              ? ''
              : item.title
          "
        >
          ...
          <van-cell
            class="already-img diyform-textarea "
            :border="false"
            v-if="item.data_type == 'diyduoxing' || item.data_type == 'diyduoxingwenben'"
            style="border: solid 1px #ccc; border-radius: 4px;"
          >
...
export default {
  props: ["datas", "form_id", "form_data_id", "status", "thumb", "description","identification"],
  data() {
    // var now = new Date();
