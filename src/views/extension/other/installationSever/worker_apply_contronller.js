...
export default {
  ...
  methods: {
    init() {
      ...
      this.apply.id_card_image_front = "";
      this.apply.id_card_image_back = "";
      this.apply.user_comment = "";
    ...
    getData() {
      let that = this;
      $http.get("plugin.live-install.frontend.worker-apply.getWorkerApply", {}, "加载中...").then(
        function(response) {
          if (response.result === 1) {
            that.diyName = response.data.another_name.worker_name;
            that.fun.setWXTitle(that.diyName + "申请");
            that.apply_id = response.data.apply_data.apply_id;
            that.o2o_info = response.data.apply_text;
            that.bannerImg = response.data.slide_list;
          } else {
            Toast(response.msg);
          }
        },
    ...
    confirmLocation(data) {
      this.apply.lng = data.point.lng;
      this.apply.lat = data.point.lat;
      this.apply.address = data.point.address;
      this.addressInfo = data.title;
    },

    // 提交数据
    setApplyData() {
      // console.log(this.apply)
      if (this.fun.isTextEmpty(this.apply.name)) {
        Toast("请填写真实姓名");
        return;
      }
      ...
      if (this.fun.isTextEmpty(this.apply.id_card_number)) {
        Toast("请填写身份证号");
        return;
      }
      if (this.fun.isTextEmpty(this.apply.id_card_image_front)) {
        Toast("请上传身份证正面");
        return;
      }
      if (this.fun.isTextEmpty(this.apply.id_card_image_back)) {
        Toast("请上传身份证背面");
        return;
      }
      ...
      let that = this;
      this.apply.other_image = this.fileList1;
      let json = this.apply;
      let url = "plugin.live-install.frontend.worker-apply.addApply";
    ...
    cancelApply() {
      let that = this;
      $http
        .get("plugin.live-install.frontend.worker-apply.cancelApply", { apply_id: this.apply_id }, "..")
        .then(response => {
          if (response.result == 1) {
            Toast(response.msg);
            that.goToIndex();
    ...
    getLocation() {
      ...
      function onComplete(obj) {
        ...
        that.apply.address = obj.formattedAddress;
      ...
      axios
        ...
        .then(response => {
          ...
          if (responseData.result === 1) {
            ...
            case 5:
              //身份证正面
              That.apply.id_card_image_front = responseData.data.img_url;
              break;
            case 6:
              //身份证反面
              That.apply.id_card_image_back = responseData.data.img_url;
              break;
            default:
              return;
            }
  ...
  components: { location, yz_uploader }
};
