...
<template>
  <div id="worker_apply">
    <div class="supplier">
      ...
      <van-swipe class="my-swipe" indicator-color="white">
        <van-swipe-item v-for="(image, index) in bannerImg" :key="index">
          <img :src="image.slide" style="width: 100%; max-height: 50vh;" @click.stop="gotoLink(image.link)" />
        </van-swipe-item>
      </van-swipe>
      <div class="main" v-if="apply_id == 0">
        <!-- 填写申请信息 -->
        <van-cell-group :border="false" class="store_apply top-info">
          ...
          <van-cell :border="false" center class="otherTitle">
            ...
            <input slot="default" type="text" placeholder="请输入姓名" v-model="apply.name" />
          </van-cell>
          ...
          <van-cell :border="false" center class="otherTitle">
            ...
            <input slot="default" type="text" @click.stop="addressShow = true" v-model="addressName" readonly placeholder="请选择所在区域" />
            <template #right-icon>
          ...
          <van-cell :border="false" center class="otherTitle">
            ...
            <input slot="default" type="number" placeholder="请输入身份证号" v-model="apply.id_card_number" />
          </van-cell>
          ...
          <van-cell :border="false">
            ...
            <span slot="default" style="display: flex; justify-content: flex-end; align-items: center;">
              ...
              <div class="" style="" @click.stop="clickLocation">{{ addressInfo ? "切换" : "自动定位" }}</div>
            </span>
          </van-cell>

          <div style="padding: 0 12px; text-align: left;">
            <p class="header_class">上传身份证正面 <em style="color: #f86e6e;">*</em></p>
              ...
              <div class="imgflex" @click="selectIndex(5)">
                <van-uploader :after-read="onRead" :max-size="photosize" @oversize="onphotosize" style="width: 100%;">
                  <div style="padding: 0.25rem 1rem; width: 100%; height: 11rem;">
                    <img :src="apply.id_card_image_front ? apply.id_card_image_front : 'https://www.yunzmall.com/min_img/bg_idcard_down@2x.png'" style="width: 100%;" class="avatar" />
                  </div>
                </van-uploader>
              </div>
            </div>
            <p class="header_class">上传身份证背面 <em style="color: #f86e6e;">*</em></p>
            <div class="otherphoto" style="padding-bottom: 0.625rem;">
              <div class="imgflex" @click="selectIndex(6)">
                <van-uploader :after-read="onRead" :max-size="photosize" @oversize="onphotosize" style="width: 100%;">
                  <div style="padding: 0.25rem 1rem; width: 100%; height: 11rem;">
                    <img :src="apply.id_card_image_back ? apply.id_card_image_back : 'https://www.yunzmall.com/min_img/bg_idcard_up@2x.png'" style="width: 100%;" class="avatar" />
                  </div>
                </van-uploader>
              </div>
            </div>
            <p class="header_class">其他图片</p>
      ...
      <!--申请后展示-->
      <div class="success" v-if="apply_id != 0">
        <div class="ico"><i class="fa fa-check"></i></div>
        <div class="text">已提交申请，等待审核通过...</div>
        <a @click="cancelApply()">
          <div class="sub">取消申请</div>
        </a>
      </div>
      <!--审核通过后展示，文字后台设置 -->
      <!-- <div class="success" v-if="apply_id == 3">
        <div class="ico"><i class="fa fa-check"></i></div>
    ...
    <van-popup v-model="agreementShow" closeable :style="{ height: '50%', width: '18rem' }" round>
      ...
      <div class="agreement-box" v-html="o2o_info" style="text-align: left; font-size: 0.875rem; line-height: 1.25rem; padding: 0.875rem;"></div>
    </van-popup>
