<template>
  <div id="supplier">
    ...
    <c-header :slides="slides" :category="category" @watchSort="sortOut" @bindCategory="bindCategory"></c-header>
    ...
    <div class="main">
      <dl v-for="(item, i) in goods" :key="i" @click.stop.prevent="gotoDetail(item.goods_id)">
        ...
        >
          ...
          <div style="color: #888; font-size: 12px;" v-if="false" v-show="item.is_open_micro == 1">
...
export default {
  ...
  methods: {
    ...
    //首页
    getIndexData() {
      $http.get("plugin.supplier.frontend.shop.index.index", { sid: this.$route.params.id }, "加载中...").then(res => {
        if (res.result == 1) {
    ...
    initShare() {
      ...
      $http
        .post("member.member.wxJsSdkConfig", json).then(response => {
