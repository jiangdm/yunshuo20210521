...
"
          ...
          <div style="text-align: center;">
            <van-uploader :after-read="onRead" :max-size="photosize" @oversize="onphotosize">
              ...
              <div style="padding: 1rem; width: 10rem; height: 10rem; border: 1px dashed #333; margin: 0 auto;">
                <img
                  :src="imageUrl ? imageUrl : require('../../../assets/images/up_icon.png')"
                  style="width: 100%;"
                  class="avatar"
                />
              </div>
      ...
      <section>
        <van-field
          v-if="fill_set.company_bank == 1"
          type="number"
          label="银行卡账号:"
          placeholder="请输入银行账号"
          v-model.trim="information.bank_account"
          center
          clearable
        />
        <van-field
          v-if="fill_set.bank_username == 1"
          label="开户人姓名:"
          placeholder="请输入开户人姓名"
          v-model.trim="information.owner_name"
          center
          clearable
        />
        <van-field
          v-if="fill_set.bank_of_accounts == 1"
          label="开户银行:"
          placeholder="请输入开户银行"
          v-model.trim="information.opening_bank"
          center
          clearable
        />
        <van-field
          v-if="fill_set.opening_branch == 1"
          label="开户支行:"
          placeholder="请输入开户支行"
          v-model.trim="information.opening_branch"
          center
          clearable
        />
    
        <van-field
          v-if="fill_set.company_ali == 1"
          label="企业支付宝账号:"
          placeholder="请输入企业支付宝账号"
          v-model.trim="information.enterprise_alipay"
          center
          clearable
        />
        <van-field
          v-if="fill_set.company_ali_username == 1"
          label="企业支付宝用户名:"
          placeholder="请输入企业支付宝用户名"
          v-model.trim="information.enterprise_alipay_user"
          center
          clearable
        />
        <van-field
          ...
          v-model.trim="information.alipay"
          center
          clearable
        />

        <van-field
          v-if="fill_set.ali_username == 1"
          label="支付宝用户名:"
          placeholder="请输入支付宝用户名"
          v-model.trim="information.alipay_username"
          center
          clearable
        />

        <van-field
          v-if="fill_set.wechat == 1"
          label="微信账号:"
          placeholder="请输入微信账号"
          v-model.trim="information.weChat_account"
          center
          clearable
        />
      </section>
