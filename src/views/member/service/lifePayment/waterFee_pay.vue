<template>
    <div id="electricity" :class="'electricity'+$store.state.service.lang">
        ...
        <template>
            <div v-for="(item,i) in order_data" :key='i' id="isSubtotal">
                ...
                <div class="integral" v-show="deduction" v-for="(d,i) in deduction" :key='i'>
                    <div>
                     ...
                     <van-switch size="28px" :class="{rt:$store.state.service.lang=='ch',lf:!$store.state.service.lang=='wei'}"  v-model="d.checked" @change="discountHandle(item,d)" active-color="#f15353" inactive-color="#fff" />
                </div>
                <div class="clear"></div>
            </div>
        </template>
