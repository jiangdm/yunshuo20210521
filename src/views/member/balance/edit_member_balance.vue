<template>
    <div id="editBalance">
        ...
        <div class="content">
            ...
            <van-cell title="保险公司:" v-if="companyRadio&&isOpenCompany" is-link :value="companyNames?companyNames:'请选择保险公司名称'"  @click="showCompany"/>
            <van-cell-group>
                <van-field :clearable="true" label="备注信息: " v-model="Memoinformation" placeholder="请输入备注信息" />
            </van-cell-group>
            <div class="button">
                <van-button type="danger" round  size="large" @click="submit">{{this.$route.params.type == 'edit'?'保存':'一键续保'}}</van-button>
            </div>
        </div>
