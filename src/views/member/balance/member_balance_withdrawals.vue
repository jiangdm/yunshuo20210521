<template>
  <div id="balance_recharge">
    <c-title :hide="false"
             :text="`${fun.initWithdrawal()}`"></c-title>
      ...
      <div class="transfer_sum">
        ...
        <div class="sum">
          <div v-if="supplier">
            ...
            <input type="number"
                   v-model="service_money"
                   disabled/><em v-if="service_type == 1">%</em>
          </div>
          <div v-if="!supplier">
            {{$i18n.t("money")}}
            <input type="number"
                   v-model="info_form.balance_money"
                   placeholder="0.00"/>
          </div>
        </div>
        <p>{{ poundage }}</p>
        <p v-if="withdraw_multiple > 1"
           style="border: none; line-height: 1rem;">提现金额需要是 <em style="font-weight: bold;">{{withdraw_multiple}}</em>
          的整倍数</p>
      </div>
      <div class="recharge_way">
        <h1>{{fun.initWithdrawal()}}到</h1>
        <ul class="choose_box">
          <li v-if="withdrawToWechat"
              @click="confirm('1')">
            <i class="iconfont icon-balance_i"></i>微信
          </li>
          <li v-if="withdrawToAlipay"
              @click="confirm('2')">
            <i class="iconfont icon-balance_j"></i>支付宝
          </li>
          <li v-if="withdrawToManual"
              @click="confirm('3')">
            <i class="iconfont icon-balance_e"></i>手动{{fun.initWithdrawal()}}
          </li>
          <li v-if="eupshow"
              @click="confirm('4')">
            <i class="iconfont icon-balance_g"></i>EUP提币
          </li>
          <li v-if="hxShow"
              @click="confirm('5')">
            <i class="iconfont icon-balance_f"></i>银行卡
          </li>
          <li v-if="converge_pay"
              @click="confirm('6')">
            <i class="iconfont icon-balance_h"></i>{{fun.initWithdrawal()}}到银行卡-HJ
          </li>
          <li @click="towithdrawal(index)"
              v-for="(type, index) of withdrawType"
              :key='index'>
            <i class="iconfont"
               :class=" type == `汇聚${fun.initWithdrawal()}`
    ...
    <van-popup v-model="popupVisible"
      ...
      <form :action="action"
        ...
        <h1
          style="font-size: 16px; color: #333; padding: 1.25rem 0; line-height: 2.25rem; font-weight: normal; border-bottom: solid 0.0625rem #ebebeb;">
          {{ notice }}
        </h1>
        <input type="text"
               name="ipsRequest"
    ...
    <van-popup v-model="show2"
      ...
      <div class="withdrawal_box">
        ...
        <div class="recharge_way">
          <div
            style="display: flex; align-items: center; justify-content: space-between; padding: 20px 10px; border-bottom: 1px solid #fafafa;"
            @click="gotoList">
            <h2 style="text-align: left;">
              银行卡：{{ checkObj.bankName }}
            </h2>
    ...
    <van-popup v-model="show3"
      ...
      <div class="withdrawal_box">
        <i class="fa fa-angle-left"
           style="display: inline-block; text-align: left; font-size: 24px; float: left; line-height: 2.875rem; height: 2.875rem; padding-left: 10px;"
           @click="show3 = false"></i>
        <h2 style="display: inline-block; line-height: 2.875rem; height: 2.875rem;">
          银行卡列表
        </h2>
          ...
          <ul class="choose_box" style="padding-top: 20px;">
            <li v-for="item in withList"
              ...
              <div style="flex: 0 0 32px;">
                <van-checkbox style="position: relative;"
                              v-model="item.check"
