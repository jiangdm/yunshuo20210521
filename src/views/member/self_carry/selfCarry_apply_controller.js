...
export default {
  ...
  activated() {
    ...
    this.$nextTick(() => {
      if (self.pass_safe == 1) {
        setTimeout(() => {
          if (self.$store.state.passwordValidation == 3) {
            self.msgText = 3;
  ...
  methods: {
    //* 获取招商专员信息
    inputMerchantsCommissonerUid({ data: userId }) {
      userId = Number(this.merchantsCommissoner.investor_uid);
      if (isNaN(userId)) {
        Toast("请输入正确的用户ID");
        return;
      }
      ...
      searchMerchantsUserHandler = setTimeout(() => {
        $http
          .get("plugin.invest-people.frontend.index.search-by-member", {
            mer_uid: this.merchantsCommissoner.investor_uid
          })
          .then(({ data }) => {
            if (data === null) {
              Toast("会员不存在");
              this.merchantsCommissoner.inputUserData = null;
              return;
            }
            this.merchantsCommissoner.inputUserData = data;
          });
      }, 500);
    },
    checkStrong(sValue) {
    ...
    getData() {
      ...
      let url;
      url = "plugin.package-deliver.frontend.apply.verify";

      $http.get(url, json, "加载中...").then(
        function(response) {
          if (response.result === 1) {
            that.diyName = response.data.name;
            that.fun.setWXTitle(response.data.name + "申请");
            that.o2o_status = 1;
            that.o2o_info = response.data.agreement;
            that.bannerImg = response.data.apply_thumb;
            that.pass_safe = response.data.safe;
            that.merchantsCommissoner.turnOn = response.basic_info.globalParameter.invest_people_open;
            if(response.basic_info.globalParameter.invest_people_open){
              if(response.basic_info.globalParameter.invest_people_open===true){
                that.merchantsCommissoner.fieldName=response.basic_info.globalParameter.invest_people_name;
              }
            }
          } else {
            that.o2o_status = 2;
    ...
    setApplyData() {
      ...
      if (this.fun.isTextEmpty(this.apply.lng) || this.fun.isTextEmpty(this.apply.lat)) {
        Toast("经纬度不能为空");
        return;
      }

      if (this.fun.isTextEmpty(this.apply.business_hours_start) || this.fun.isTextEmpty(this.apply.business_hours_end)) {
      ...
      let url = "plugin.package-deliver.frontend.apply.store";
      if (this.merchantsCommissoner.turnOn) {
        if (this.merchantsCommissoner.investor_uid) {
          if (!this.merchantsCommissoner.inputUserData) {
            Toast("招商专员不存在");
            return;
          }
          json['apply']["investor_uid"] = this.merchantsCommissoner.investor_uid;
        }
      }
      ...
      axios
        ...
        .then(response => {
          ...
          if (responseData.result === 1) {
            ...
            case 4:
              //营业执照
              That.imageUrl = responseData.data.img_url;
              That.apply.aptitute[0] = responseData.data.img_url;
    ...
    toGeolocation() {
      ...
      this.$router.push(this.fun.getUrl("o2oLocation_loc", {}, { tag: "selfCarryApply" }));
    },
    sureStartTime(e) {
      this.beginTime = e;
