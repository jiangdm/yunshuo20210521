<template>
  <div id="goodsinfo" :class="[this.fun.getPhoneEnv() == 3 ? 'pcStyle' : '']">
      ...
      <ul class="addr-info">
        ...
        <template v-if="order_data.dispatch_type_id == 8 && !this.fun.isTextEmpty(order_data.package_deliver)">
          ...
          <div
            ...
            v-if="!fun.isTextEmpty(package_deliver.lng) && !fun.isTextEmpty(package_deliver.lat)"
          >
        ...
        <template v-if="order_data.dispatch_type_id != 8">
          ...
          <div v-if="jurisdiction_set != 1">
            ...
            <li style="margin-bottom: 0.5rem; font-size: 16px;" v-if="receiver">
              收件人：{{ receiver }} （{{ mobile }}）
            </li>
            <li style="color: #999;">
    ...
    <div class="tbs">
      ...
      <div
        class="kefu"
        v-if="order_data.has_many_order_goods && order_data.has_many_order_goods.length > 0"
        @click="getKefu"
      >
    ...
    <div class="detail_pay">
      <template v-for="(item, index) in order_data.button_models">
        ...
        <div
          :key="index"
          v-if="index == 2 && order_data.button_models.length > 2"
          class="order_delete red-button"
          @click.stop="showMore(order_data.order_sn)"
        >
          更多
        </div>
        <div class="trans" :key="index" v-if="index == 2 && showID == order_data.order_sn"></div>
        <div class="more-button" :key="index" v-if="index == 2 && showID == order_data.order_sn">
          <p
            v-for="(item, ind) in order_data.button_models"
            :key="ind"
            v-if="ind >= 2"
            @click="operation(item, order_data)"
    ...
    <van-dialog v-model="dialogVisible" :showConfirmButton="false">
      ...
      <section style="width: 100%; text-align: left;">
        <div style="width: 100%; height: 80px;" v-if="addresseeInfo && jurisdiction_set != 1">
    ...
    <van-popup v-model="showDFData" position="bottom" closeable round :overlay="true" :style="{ height: '80%' }">
      <c-dyPopup :datas="dfData" v-if="showDFData" :status="false"></c-dyPopup>
    </van-popup>

    <van-image-preview
      v-model="showPhotoGoodsArr"
      :images="PhotoGoodsArr"
      @change="onChangePhotoGoods"
    ></van-image-preview>
    ...
    <div class="fixed-box" v-show="!showPacket && hasPacket" :class="[fun.getPhoneEnv() == 3 ? 'pcStyle' : '']">
      <img
        @click="showPacket = true"
        class="rotate"
        :class="[{ r1: shrinkPacket }, { r2: !shrinkPacket }]"
        src="../../../assets/images/consume_packet.png"
      />
    </div>
    <!--消费红包end-->
