...
export default {
  ...
  methods: {
    ...
    getConfig() {
      $http.get("plugin.picking-card.frontend.pickingCard.getConfig", {}, "加载中...").then(
        response => {
    ...
    checkStatus() {
      $http.get("plugin.picking-card.frontend.pickingCard.checkRefundStatus", {order_id: this.order_id}, "加载中...").then(
        response => {
          if (response.result === 1) {
            if(response.data === 0) {
    ...
    setApplyData() {
      ...
      $http.post("plugin.picking-card.frontend.pickingCard.postRefund", json, "提交中").then(
        response => {
          if (response.result === 1) {
            this.$router.replace(this.fun.getUrl("pickUpCardOrderList"));
          } else {
            Toast(response.msg);
          }
        },
    ...
    onRead(e, flag, num) {
      ...
      let That = this;
      axios
        .post(this.fun.getRealUrl("plugin.picking-card.frontend.pickingCard.upload"), fd, {
