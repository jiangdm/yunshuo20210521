<template>
  <div class="page">
    <c-title :hide="false" text="提货卡"></c-title>
    ...
    <van-form @submit="onSubmit">
      <div class="content">
        <van-field
          ...
          placeholder="请输入提货密码"
          @blur="getPrice"
          :rules="[{ required: true, message: '请输入提货密码' }]"
        />
        ...
        <van-cell is-link style="padding-left: 10px; padding-right: 10px;" class="text-name">
          <span style="margin-right: 0.5rem;"><span style="color: #ee0a24;">*</span>所在城市</span>
        ...
        <van-field required v-model.trim="applyModel.address" label-width="4.5rem" label="收货地址:" placeholder="请输入详细地址" />
      </div>
      <div class="text">{{ introduction.commitment }}: {{ $i18n.t("money") }}{{ price || 0 }}</div>
      <!-- <div class="input text install">
        <div>
          <span style="color: #ee0a24;">*</span>
          <span style="margin: 0 0.2rem;">请确认是否使用{{introduction.commitment}}兑换{{introduction.filter}}</span>
        </div>
        <van-radio-group v-model="applyModel.is_filter" direction="horizontal" class="radio">
          <van-radio name="1" icon-size="0.9rem" checked-color="#f15353" class="radioItem">是</van-radio>
          <van-radio name="2" icon-size="0.9rem" checked-color="#f15353" class="radioItem">否</van-radio>
        </van-radio-group>
      </div> -->
