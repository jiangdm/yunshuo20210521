...
export default {
  ...
  methods: {
    ...
    postSelectGood() {
      console.log("进行数据提交");
      this.$router.push(
        this.fun.getUrl("microShop_selectedGoods", { goods: this.pushData })
      );
    },
    pushFn() {
      let that = this;
      ...
      $http
        .get(
          "plugin.micro.frontend.controllers.MicroShop.set-goods",
          { goods_ids: this.pushData },
          "提交中"
        )
        .then(
          function(response) {
