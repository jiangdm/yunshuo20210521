<template>
  <div id="microShop_main">
    ...
    <div class="home">
      ...
      <div class="ment"
           v-if="category">
        <c-ment :datas="category"
                :shop_id="shop_id"></c-ment>
      </div>

      <div class="my-swipe"
           v-if="bannerData.length > 0">
        <swipe style="height: 9.375rem;">
          <swipe-item v-for="(item,i) in bannerData" :key='i'>
            <template v-if="item.thumb">
              <img :src="item.thumb"
                   width="100%" @click="tohref(item.link)" />
            </template>
            <template v-if="!item.thumb">
      ...
      <div class="goodsList"
           v-if="goodsListData&&!fun.isTextEmpty(goodsListData)">
        <c-goodsList :goods="goodsListData"
                     :num="6"
                     text="商品列表"></c-goodsList>
      </div>
    ...
    <!-- 页面底部的导航栏 -->
    <div class="footer" :style="{width: fun.getPhoneEnv() == 3 ? '375px' : ''}">
      <ul>
        ...
        <router-link :to="fun.getUrl('micro_shop_share_category',{fromHome: 1})">
          <li class="na">
