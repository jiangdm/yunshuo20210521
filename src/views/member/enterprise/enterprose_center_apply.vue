  ...
  <div id="areaAgent">
    ...
    <div class="success" v-if="status == '1'">
      ...
      <a @click="goEnterproseCentre()">
        <div class="sub">去{{title || '招商中心'}}</div>
      </a>
    </div>
    <!--可申请-->
    <div class="content" v-if="status == '-1'">
      <template v-if="config_status == '1'">
        <p class="title">申请{{merchant_center || '招商中心'}}</p>
        <div class="text">
          开启会员关系链，如果成为{{merchant_center || '招商中心'}}，那么只要您的粉丝成为供应商，即可获得供应商的分红哦，快来申请吧~
        </div>
      ...
      <template v-if="config_status == '2'">
        <p class="title">申请{{merchant_center || '招商中心'}}</p>
        <div class="text">
          团队会员人数 <span>{{ config2.total }}</span
      ...
      <template v-if="config_status == '4'">
        <p class="title">申请{{merchant_center || '招商中心'}}</p>
        <div class="text">
          自购订单金额 <span>{{ config4.total }}</span
          >元 您还差 <span>{{ config4.diff }}</span
          >元
        </div>
        <template v-if="independent_backend == 1">
          <van-field v-model="username" label="登录账号" placeholder="登录账号" />
      ...
      <template v-if="config_status == '5'">
        <p class="title">申请{{merchant_center || '招商中心'}}</p>
        <div class="text">
          团队总消费金额 <span>{{ config5.total }}</span
          >元 您还差 <span>{{ config5.diff }}</span
          >元
        </div>
        <template v-if="independent_backend == 1">
          <van-field v-model="username" label="登录账号" placeholder="登录账号" />
