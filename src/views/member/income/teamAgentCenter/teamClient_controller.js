...
export default {
  ...
  methods: {
    ...
    showDetail(obj) {
      ...
      let that = this;
      $http.get('plugin.team-dividend.api.team.getInfo', {uid: obj.has_one_member.uid}).then(function (response) {
    ...
    getTab() {
      let that = this;
      $http.get('plugin.team-dividend.api.team.getColumns', {}).then(function (response) {
        if (response.result === 1) {
    ...
    getData() {
      ...
      that.page = 1;
      $http.get('plugin.team-dividend.api.team.getList', {id: that.tabID}, "加载中...").then(function (response) {
        if (response.result === 1) {
