			...
			<div class="item" v-for="(item,key) in listData" :key="key" @click="setLastItemKey(key)">
				...
				<template v-if="item.project_shell_type==2 || item.project_shell_type==1">
					...
					<div class="leftc">
						...
						<div class="c5 cc" v-if="item.spell_type==0">预约金额：<span class="price">{{$i18n.t('money')}}{{item.actual_amount}}</span></div>
						<div class="c5 cc" v-else >实际消费：<span class="price">{{$i18n.t('money')}}{{item.actual_amount}}</span></div>

						<div class="c5 cc" v-if="item.next_amount">下个阶梯：<span class="price">{{$i18n.t('money')}}{{item.next_amount}}</span></div>
					</div>
				</template>
				...
				<template v-if="item.project_shell_type==3">
					...
					<div class="leftc">
						...
						<div class="c5 cc">拼单金额：<span class="price">{{$i18n.t('money')}}{{item.shell_amount}}</span></div>
...
<style lang="scss" rel="stylesheet/scss" scoped>
  ...
  .page { background: rgb(246, 248, 252); min-height: 100vh; padding-bottom: 3.125rem; }
