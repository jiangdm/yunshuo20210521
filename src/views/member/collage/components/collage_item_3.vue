    ...
    <div class="leftc">
      ...
      <div class="c4 cc" style="display: flex;">释放时间：
        <template v-if="((item.lock_update_at*1000)-currentTime)>0">
          <van-count-down style="font-size: 0.75rem; color: #767676;" :time="((item.lock_update_at*1000)-currentTime)"
      ...
      <div class="c5 cc">拼单金额：<span class="price">￥{{item.shell_amount}}</span></div>
      <div class="c5 cc" v-if="item.next_amount">下个阶梯：<span class="price">￥{{item.next_amount}}</span></div>

      <div class="c5 cc" v-if="item.has_one_relation && item.has_one_relation.has_one_order">分红阶段：<span class="price">￥{{item.has_one_relation.has_one_order.shell_amount}}</span>
      </div>
...
export default {
  ...
  methods: {
    ...
    gotoDetProject() {
      this.$router.push(
        this.fun.getUrl("tacitDetproject", {}, { detId: this.item.id })
      );
    },
    gotoEditProject() {
      this.$router.push(
        this.fun.getUrl("tacitEditproject", {}, { detId: this.item.id })
      );
    },
    addCollage() {
    ...
    .leftc {
      ...
      .c1 {
        ...
        .ope {
          width: 1.469rem;
          height: 1rem;
          background-image: url(https://www.yunzmall.com/min_img/pd_more@2x.png);
