        ...
        <template v-for="(item,key) in diydata">
          <template v-if="item.type == 'diyinput'">
            ...
            <div class="inputWrapper" :key="key">
              ...
              <input type="text" class="input" readonly="readonly" :required="item.data.tp_must == 1"
                     v-model="item.value" :placeholder="item.data.placeholder">
          ...
          <template v-if="item.type == 'diyimg'">
            ...
            >
              ...
              <div
                :key="key"
                class="clearfloat"
                style="width: 100%; text-align: center; line-height: 5rem; padding: 1rem; padding-left: 0; padding-top: 0.4rem;"
                :style="{ backgroundColor: datas.params.bgcolor }"
              >
                <template v-for="(val,ind) in item[item.name + 'Value2']">
                  <div class="photobox" style="float: left; max-width: 25%;" :key="ind">
  ...
  methods: {
    ...
    getDiyForm() {
      ...
      if (this.fun.isTextEmpty(this.form_id)) {
        console.log(
          "error:: 报单form_id为空！！ url:src\components\temp\form.vue"
        );
        return;
      }
      $http
