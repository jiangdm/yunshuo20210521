<template>
  <div id="time-limit" :style="'background:'+datas.params.bgcolor">
    <div id="rush-buy">
      <!-- 商品组标题 -->
      ...
      <div class="single-row" v-if="datas.params.style=='100%' && showTime(item.start_time)?true:false"
           v-for="(item,k) in datas.data" @click.stop.prevent="buy(item)" :key="k">
        <div class="img" :style="{ backgroundImage:'url('
          ...
          <div class="time-box" v-if="datas.params.countdown">
            <template v-if="new Date()>new Date(item.start_time)">
              <van-count-down :time="fun.getTimeDifference(fun.getTimestamp(item.end_time))" format="DD天HH时mm分ss秒" style="color: #fff;"/>
            </template>
            <template v-else>
              <span>距开始：</span>
              <van-count-down :time="fun.getTimeDifference(fun.getTimestamp(item.start_time))" format="DD天HH时mm分ss秒" style="color: #fff;"/>
            </template>
          </div>
        </div>
        <ul class="text">
      ...
      <div class="double-row" v-if="datas.params.style=='49.5%'?true:false">
        <dl class="list-box" v-for="(item,index) in datas.data" :key="index" v-if="showTime(item.start_time)"
      ...
      <div class="three-row" v-if="datas.params.style=='33.3%'?true:false">
        <dl class="list-box" v-for="(item,i) in datas.data" v-if="showTime(item.start_time)"
            @click.stop.prevent="buy(item)" :key='i'>
          <dt class="img" :style="{ backgroundImage:'url('
                     +
                     (item.img?item.img:require('../assets/images/loading.jpg'))
                     +')'} ">
      ...
      <div class="across-row" v-if="datas.params.style=='hp'?true:false">
        <div class="list" v-for="(item,j) in datas.data" :key='j' v-if="showTime(item.start_time)" @click.stop.prevent="buy(item)">
          <div class="img">
            <img :src="item.img?item.img:require('../assets/images/loading.jpg')">
            <div class="sale-img" :class="datas.params.option"></div>
