<template>
  <div id="video-list">
    <img
      style="width: 5rem; padding-top: 3rem; display: inherit;"
...
<script>
export default {
  props: ["list","showLoading","card_id", "isDiy",'isLoadMore','id'],
  data() {
    return {
      newArray1: [],
      newArray2: [],
  ...
  methods: {
    ...
    toVideo(item) {
      ...
      if(this.isDiy) {
        this.$router.push(
          this.fun.getUrl("VideoDetail", {}, { vid: item.id })
        );
      }else {
        this.$router.push(
          this.fun.getUrl("VideoDetail", {}, { vid: item.id, page: page, from: 'card',card_id:this.card_id })
        );
      }
    },
    sort(arg, list) {
        ...
        that.getImg(
          ...
          function(w, h, r) {
            ...
            if (document.getElementsByClassName(`zq-waterfall-left-video${that.id}`)[0]) {
              boxA = document.getElementsByClassName(`zq-waterfall-left-video${that.id}`)[0]
                .clientHeight;
            }

            if (document.getElementsByClassName(`zq-waterfall-right-video${that.id}`)[0]) {
              boxB = document.getElementsByClassName(`zq-waterfall-right-video${that.id}`)[0]
                .clientHeight;
            }
