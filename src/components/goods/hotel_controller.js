...
export default {
  ...
  methods: {
    ...
    appShare(data) {
      var that = this;
      var _title = that.fun.isTextEmpty(that.goods_info.has_one_share.share_title)
        ? that.goods_info.title
        : that.goods_info.has_one_share.share_title;
      var _link = document.location.href + '&share_tag=2';
      _link = that.fun.isMid(_link, data.info.uid);

      var _imgUrl = that.fun.isTextEmpty(that.goods_info.has_one_share.share_thumb)
    ...
    postShow() {
      if (!this.fun.isTextEmpty(this.posterImg)) {
        ...
        $http
          .get(
            'goods.goods-poster.new-goods-poster',
  ...
  components: { Swipe, SwipeItem, cMyswipe, cComment, cCommentlist, cLive, yzGoodsposter,floatOrder },
  watch: {
    scroll() {
      this.loadSroll();
    },
