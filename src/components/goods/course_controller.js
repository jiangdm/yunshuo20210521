...
export default {
  ...
  mounted() {
    if(this.$route.name !== 'CourseDetail') {
      ...
      this.getDetailInfo();
      if(this.goods_info.get_goods && this.goods_info.get_goods.point_goods) {
        this.point_goods = this.goods_info.get_goods.point_goods;
      }
    }
  },
  filters: {
        ...
        .then(response => {
          if (response.result === 1) {
            this.info = JSON.parse(
              this.secret(response.data, "zoQUA@257ggflNmMppKzcU7QgYyp!tU&h54VNN3u!PAU$8FxBW", true)
            );
            this.livezhibo();
          } else {
            Toast(response.msg);
      ...
      $http
        ...
        .then(response => {
          if (response.result === 1) {
            ...
            try {
              this.shopName = response.basic_info.home.mailInfo.name;
            } catch (error) {
              console.log(error);
            }
            ...
            for (let j = 0; j < has_many_dig_chapter.length; j++) {
              if (has_many_dig_chapter[j].has_many_chapter.length > 0 && this.chapter_id) {
                first_playActive = j;
