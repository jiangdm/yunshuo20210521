...
export default {
  ...
  activated() {
    ...
    if (!this.fun.isTextEmpty(this.$route.query.nocart) || this.$route.query.fromStock) {
      this.nocart = false;
    } else {
      this.nocart = true;
    }
  ...
  mounted() {
    ...
    console.log(this.isBegTime, this.isBuy, "标识标识");
  },
  ...
  methods: {
    ...
    getGoodsPage(data) {
      ...
      if(data.cservice.toString().indexOf("chatWindow") > -1){
        this.cservice +="&goods_id="+this.goodsInfo.id;
      }
      this.luck_draw = data.luck_draw || {};
    ...
    //获取数据
    getData(data) {
      console.log(this.gjzMagnification);
      console.log(data, "获取数据");
      if (data.hasOwnProperty("pack_fixed_price") == true && data.pack_fixed_price != {}) {
        this.pack_price = data.pack_fixed_price.pack_price;
        this.goods_count = data.pack_fixed_price.goods_count;
        this.ActivityId = data.pack_fixed_price.id;
      ...
      if (data.prdocut_market) {
        // 应用市场
        this.prdocut_market = data.prdocut_market;
      }
    ...
    initPopView() {
      ...
      if (this.goodsInfo.has_option == 1) {
        ...
        if (this.goodsInfo.point_goods) {
          this.popPrice = this.goodsInfo.point_goods.point; //设置默认价格
        } else {
          ...
          for (let ind = 0; ind < that.goodsInfo.has_many_specs.length; ind++) {
            for (let index = 0; index < that.goodsInfo.has_many_specs[ind].specitem.length; index++) {
              if (that.goodsInfo.has_many_specs[ind].specitem[index].id == _specs) {
                ...
                if (this.goodsInfo.point_goods) {
                  that.popPrice = that.goodsInfo.point_goods.point_option[_activeSkuIndex].goods_point; //设置价格
                } else {
      ...
      $http.post("member.member.wxJsSdkConfig", json).then(
        response => {
          ...
          if (response.result === 1) {
            ...
            //   this.initCservice(response.data.shop.cservice, response.data.shop.service_QRcode, response.data.shop.service_mobile); //重新赋值
            // }
            // else {
            //   $http
            //     .get(
    ...
    getCart(goods) {
      ...
      that.cartsNum = 0;
      that.cartsTotal = 0;
      $http.get("plugin.store-cashier.frontend.shoppingCart.member-cart.index", { store_id: this.$route.params.store_id }, "加载中...").then(
        response => {
          if (response.result == 1) {
