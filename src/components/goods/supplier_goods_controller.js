...
export default {
  ...
  methods: {
    ...
    getData(data) {
      ...
      if (data.hasOwnProperty('pack_fixed_price') == true && data.pack_fixed_price == {}) {//商品未参加一口价
      ...
      if (!this.fun.isTextEmpty(data.has_one_goods_limit_buy)) {
        ...
        this.endTime = data.has_one_goods_limit_buy.end_time;
        ...
        response => {
          if (response.result === 1) {
            ...
            if (!this.fun.isTextEmpty(response.data.shop)) {
              this.initCservice(
                response.data.shop.cservice,
                response.data.shop.service_QRcode,
                response.data.shop.service_mobile
              ); //重新赋值
            }
            // else {
            //   $http
    ...
    postShow() {
      if (!this.fun.isTextEmpty(this.posterImg)) {
        ...
        $http
          .get('goods.goods-poster.new-goods-poster', { id: this.$route.params.id }, '...加载中')
          .then(response => {
            toastPoster.clear();
            if (response.result == 1) {
    ...
    updateCart(id, num) {
      ...
      $http
        .get(
          ...
          { num: num, store_id: this.$route.params.store_id, goods_id: id },
          '加载中...'
        )
        .then(response => {
          console.log('result', response);
    ...
    delItemByCart(cart_id) {
      ...
      $http
        .get(
          ...
          { store_id: this.$route.params.store_id, goods_id: cart_id },
          '删除中...'
        )
        .then(response => {
          if (response.result == 1) {
