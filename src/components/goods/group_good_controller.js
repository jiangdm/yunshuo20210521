...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.teamInfo = { newword: false, totalPage: 0, total: 0, page: 1, listData: [] };
    },
    handleHtml($event) {
      if ($event.target) {
        ...
        response => {
          if (response.result === 1) {
            ...
            this.fun.wxShare(
              ...
              {
                title: this.fun.isTextEmpty(this.gooddatas.share_title) ? this.gooddatas.title : this.gooddatas.share_title,
      ...
      $http.get(_url, json, "加载中").then(
        response => {
          if (response.result === 1) {
            ...
            if (this.teamInfo.listData.length > 0) {
              this.show2 = true;
            } else {
              Toast({
                message: "暂无更多可参与的团",
    ...
    setGroupInfo(_id) {
      ...
      for (let i = 0; i < this.gooddatas.has_many_options.length; i++) {
        ...
        if (_optionSpecs == optionsId) {
          // options.push(this.goodsInfo.has_many_options[i]);
          that.selectGroupInfo = this.gooddatas.has_many_options[i].has_many_option_level;
    ...
    showGroupMore() {
      ...
      if (this.gooddatas.status == "1") {
        //活动已开启
        if (this.teamInfo.newword == false) {
          this.teamInfo.newword = true;
          this.getTeamList();
          this.show2 = false;
          return;
        } else if (this.teamInfo.newword == true && this.teamInfo.listData.length == 0) {
          Toast({
            message: "暂无更多可参与的团",
            duration: 1000
          });
    ...
    postShow() {
      ...
      $http
        .get("plugin.fight-groups.frontend.controllers.group-poster.generate-goods-poster", { id: this.activity, store_id: this.$route.params.store_id }, "加载中")
        .then(response => {
          toastPoster.clear();
          if (response.result == 1) {
    ...
    share(data) {
      ...
      wx.ready(function() {
        ...
        _titles = that.fun.isTextEmpty(that.gooddatas.share_title) ? that.gooddatas.title : that.gooddatas.share_title;

        // _links = that.gooddatas.share_url;
        _links = that.fun.isMid(document.location.href, data.info.uid);
    ...
    hideIdexbb() {
      ...
      for (let i = 0; i < this.teamInfo.listData.length; i++) {
        if (this.teamInfo.listData[i].end_time <= nowTimes) {
          if (i == 0) {
            this.teamInfo.listData.shift(); //删除并返回数组的第一个元素
            return this.teamInfo.listData;
          } else if (i == length - 1) {
            this.teamInfo.listData.pop(); //删除并返回数组的最后一个元素
            return this.teamInfo.listData;
          } else {
            this.teamInfo.listData.splice(i, 1); //删除下标为i的元素
            return this.teamInfo.listData;
          }
        }
      }
    ...
    goToAdress() {
      let point = this.fun.bd_decrypt(this.gooddatas.store.store.longitude, this.gooddatas.store.store.latitude);
  ...
  computed: {
    ...
    showOpeonBtn() {
      ...
      if (that.gooddatas.menu_show == 0) {
        return true;
      } else if (that.gooddatas.menu_show == 1 && that.gooddatas.has_many_team_count < that.gooddatas.hide_num) {
        return true;
      } else {
        return false;
      }
