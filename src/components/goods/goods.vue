      ...
      <div ref="onePage" id="onePage" style="overflow: hidden; min-height: 100vh;">
        <div style="-webkit-overflow-scrolling: touch; min-height: 101vh;" id="main">
          ...
          <div class="optional" @click="toFixedPrice()" v-if="hasActivity">
            ...
            <span class="go-group">
              去凑单
              <i class="iconfont icon-advertise-next icon-arrow"></i>
            </span>
          </div>
          ...
          <div class="member-box" v-if="prdocut_market.plugin && prdocut_market.plugin.length > 0" @click="PopupShow = true">
            <span class="optiondiv">
              应用
              <em style="padding-left: 0.625rem; color: #333;">{{ prdocut_market.apps }}</em>
            </span>
            <span>
              <em class="application">插件</em>
            </span>
          </div>
          <div class="act-box">
            ...
            <ul class="act" @click="gotohuiyuan" v-if="goods_info.selfbuy_discount" style="height: auto; align-items: center;">
              ...
              <li class="act2" style="height: auto;">
                <div v-if="goods_info.selfbuy_discount.this_level_discount_price.show == 1">{{ goods_info.selfbuy_discount.this_level_discount_price.content }}</div>
                <div v-if="goods_info.selfbuy_discount.next_level_discount_price.show == 1">{{ goods_info.selfbuy_discount.next_level_discount_price.content }}</div>
              </li>
              <li class="act3">
                <span></span>
              </li>
...
"
      ...
      <van-popup v-model="popupSpecs" :style="{ overflow: 'visible' }" position="bottom" class="mint-popup-4 pcStyle_mint" closeOnClickModal="true">
        <div class="specifications">
          ...
          <div class="shopimg">
            ...
            <div class="right">
              <div class="price" v-if="goodsInfo.point_goods">
                <span id="option_price">
                  {{ popPrice }}
                </span>
                <span class="point-text" style="font-size: 13px;">{{ integral }}</span>
              </div>
          ...
          <van-button type="danger" size="small" id="btsmall" @click="submitAction" v-show="popStock >= 1">确认</van-button>
          <van-button type="danger" color="#999999" size="small" disabled id="btsmall" v-show="popStock <= 0">确认</van-button>
        </div>
      </van-popup>
      ...
      <div id="foot">
        <!--供应商-->
        <div class="addfav" @click="gotoSupplier" v-if="isCup && goods_type == 'supplierGoods'">
          <template>
            <ul>
              <li>
        ...
        <!--平台商品-->
        <div class="addfav" @click="gotoCart" v-if="isCup && goods_type != 'supplierGoods'">
          <template>
            <ul>
              <li>
                <i class="iconfont icon-info_store"></i>
        ...
        <div v-if="isCup && nocart && goods_info.get_goods && goods_info.get_goods.plugin_id != 72" class="addfav">
        ...
        <div
          ...
          @click="addCart"
          v-if="isCup && !isBegTime && nocart && !isPendingOrder && goods_type !== 'appointment_goods' && goods_info.get_goods && goods_info.get_goods.plugin_id != 72"
        >
          加入购物车
        </div>
