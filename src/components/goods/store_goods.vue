    ...
    <div id="goods" v-show="showGood" :class="[this.fun.getPhoneEnv() == 3 ? 'pcStyle' : '']">
      ...
      <div ref="onePage" id="onePage" style="overflow: hidden; min-height: 100vh;">
        <div style="-webkit-overflow-scrolling: touch; min-height: 101vh;" id="main">
          ...
          <float-order v-if="showGood" :broadcast_data="goods_info_store.broadcast_data"></float-order>
          ...
          <div id="goodsInfo_box">
            <van-row id="nameInfo">
              ...
              <van-col id="price" class="price_sum" v-if="!isGoodsLove" style="flex: 1;">
                <div class="show_price_box" v-if="goodsInfo.goods_show && goodsInfo.goods_show.store_goods_detail_style == 1">
                ...
                <template v-if="!goodsInfo.goods_show || goodsInfo.goods_show.store_goods_detail_style != 1">
                  {{ $i18n.t("money") }}
                  ...
                  <span id="price-num" class="sum" v-else>{{ goodsInfo.has_option == 1 ? goodsInfo.min_price + "-" + goodsInfo.max_price : goodsInfo.price }}</span>
            ...
            <div id="price-num"
              ...
              原价:{{$i18n.t('money')}}{{ goodsInfo.market_price }}
            </div>
          </van-col>
          <van-col :span="24"
                   id="price"
            ...
            <van-col :span="5"
              ...
              <div id="price-num"
                   style="font-size: 0.8rem;font-weight: 600;text-align: center;">
                {{$i18n.t('money')}}{{ goodsInfo.vip_level_status&&goodsInfo.vip_level_status.status==1?goodsInfo.vip_level_status.word:goodsInfo.price }}
              </div>
            </van-col>
            <van-col :span="19"
                     class="sale"
              ...
              <van-col :span="24" id="stockcontainer">
                ...
                <van-col :span="12" class="sale"> 销量:{{ salesCount }}</van-col>
              </van-col>
              <van-col :span="24" v-if="is_o2o & (!isBuy || (isBuy && !isBegTime))" style="position: relative; padding: 0 0.75rem;">
                <div id="select" v-if="o2oCount > 0 && goods_info.has_option !== 1">
                  <div id="left" @click="updateCart(goodsInfo.id, -1)" v-if="o2oCount > 0">
                    -
                  </div>
                ...
                <div class="spec_btn" :style="{ bottom: isGoodsLove ? '2.3rem' : '0.3rem' }" v-if="o2oCount >= 0 && goods_info.has_option === 1">
                  ...
                  >
                  <button style="width: 5.85rem; height: 2.1rem;" @click="addGood" v-if="!goods_info.vip_level_status || goods_info.vip_level_status.status == 0">选规格</button>
                </div>
          ...
          <div class="act-box">
            ...
            <!-- 自购省 -->
            <ul class="act" @click="gotohuiyuan" v-if="goods_info_store.selfbuy_discount" style="height: auto; align-items: center;">
              <li class="act1" style="color: #999;">自购省</li>
              <li class="act2" style="height: auto;">
                <div v-if="goods_info_store.selfbuy_discount.this_level_discount_price.show == 1">
                  {{ goods_info_store.selfbuy_discount.this_level_discount_price.content }}
                </div>
                <div v-if="goods_info_store.selfbuy_discount.next_level_discount_price.show == 1">
                  {{ goods_info_store.selfbuy_discount.next_level_discount_price.content }}
                </div>
              </li>
              <li class="act3"><span></span></li>
              <i class="fa fa-angle-right" v-if="goods_info.selfbuy_discount && goods_info.selfbuy_discount.link_to_level_page && goods_info.selfbuy_discount.link_to_level_page.show == 1"></i>
            </ul>
                  ...
                  <div v-if="active == '0' && toker">
                    <div class="meal">
                      <div style="font-size: 0.88rem;">
                        <span>核销日期：</span>
                        <!-- <span>{{customer_development.effective_time == 0?'永久有效':'购买后'+customer_development.effective_time+'天内有效（限周一，周二，周四使用）'}}</span> -->
                        <span v-if="effective_time == 0">永久有效</span>
                        <span v-if="effective_time == 1 && effective_day.length > 0">
                      ...
                      <template v-if="once_list.length > 0">
                        ...
                        <div class="onelis" v-for="(item, ind) in once_list" :key="ind">
                          <div class="oneleft">
                      ...
                      <template v-if="many_list.length > 0">
                        ...
                        <div class="onelis" v-for="(item, ind) in many_list" :key="ind">
                          <div class="oneleft">
                            <img :src="item.full_img_url" alt="" />
                          </div>
                          <div class="oneright">
      ...
      <van-popup v-model="popupSpecs" :style="{ overflow: 'visible' }" position="bottom" class="mint-popup-4" closeOnClickModal="true">
        <div class="specifications">
          ...
          <van-button size="small" id="btsmall" type="danger" v-show="popStock >= 1" @click="submitAction">确认 </van-button>
        ...
        <div class="spec_box">
          ...
          <div class="add_car">
            <ul class="car_btn">
              ...
              <li class="right">
                <span v-show="!popNum && popStock >= 1 && !toker" @click.stop="submitAction1">加入购物车</span>
                <span v-show="popStock <= 0" style="background: #999;">库存不足</span>
                <!-- 拓客卡 -->
                <span @click.stop="submitAction1" v-show="!popNum && popStock >= 1 && toker">立即买</span>
              </li>
      ...
      <!-- 未添加商品 -->
      <div class="Go_settle" v-show="carts.length === 0 && showDis && !notLogin && !toker">
      ...
      <!-- 已添加商品 -->
      <div class="Go_settle complete" v-show="carts.length > 0 && showDis && !notLogin">
          ...
          <div>
            <div class="goodsList">
              <div class="goods" v-for="(item, index) in carts" @click.stop="goToGoodsO2O(item)" :key="index">
                ...
                <div class="addBtn">
                  <i v-if="goods_info.has_option !== 1" class="iconfont icon-store_reduceLine" @click.stop="updateCart(item.goods_id, -1)"></i>
                  <i v-if="goods_info.has_option === 1" class="iconfont icon-store_reduceLine" @click.stop="updateCart1(item.id, -1)"></i>
                  <input v-if="goods_info.has_option !== 1" type="number" v-model.lazy="item.total" @focus="clickCount(item.total)" @blur="changeCount(item.total, item.goods_id)" @click.stop />
                  <input v-if="goods_info.has_option == 1" type="number" v-model.lazy="item.total" @focus="clickCount(item.total)" @blur="changeCount(item.total, item.id)" @click.stop />
                  <i v-if="goods_info.has_option !== 1" class="iconfont icon-store_plus" @click.stop="updateCart(item.goods_id, 1)"></i>
                  <i v-if="goods_info.has_option === 1" class="iconfont icon-store_plus" @click.stop="updateCart1(item.id, 1)"></i>
                </div>
              </div>
            </div>
      ...
      <van-popup v-model="showVideoLink" round closeable>
        <div class="video-link-copy">
          ...
          <img src="https://www.yunzmall.com/min_img/goods/videolink.png" alt="" class="video-link-copy-img" />
        </div>
      </van-popup>
    </div>
  </transition>
