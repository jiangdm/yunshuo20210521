    ...
    <div id="goods" v-show="showGood" :class="[this.fun.getPhoneEnv() == 3 ? 'pcStyle' : '']">
      ...
      <div ref="onePage" id="onePage" style="overflow: hidden; min-height: 100vh;">
        <div style="-webkit-overflow-scrolling: touch; min-height: 101vh;" id="main">
          <viewer :images="goodsInfo.thumb_url" class="section" style="position: relative;">
            <c-myswipe :style="{ height: this.fun.getPhoneEnv() == 3 ? '375px' : '100vw' }" :pagination-visible="true" :slides="goodsInfo.thumb_url" :repeating="true" :auto="0">
          ...
          <float-order v-if="showGood" :broadcast_data="goods_info.broadcast_data"></float-order>
          ...
          <li class="middle-line"><small>{{$i18n.t('money')}}</small><b>3.2</b>&nbsp;/天</li> -->
            ...
            <div class="time" v-if="!isBegTime">
              ...
              <van-count-down :time="fun.getTimeDifference(endTime)" class="bottom_time" format="DD 天 HH 时 mm 分 ss 秒" @finish="timeEnd">
                <template #default="timeData">
                  <em style="padding: 0.25rem 0.125rem; border-radius: 0.125rem;">{{ timeData.days }}</em
                  ><span style="color: #fff;">天</span> <em style="padding: 0.25rem 0.125rem; border-radius: 0.125rem;">{{ timeData.hours < 10 ? `0${timeData.hours}` : timeData.hours }}</em
                  ><b>:</b> <em style="padding: 0.25rem 0.125rem; border-radius: 0.125rem;">{{ timeData.minutes < 10 ? `0${timeData.minutes}` : timeData.minutes }}</em
                  ><b>:</b>
                  <em style="padding: 0.25rem 0.125rem; border-radius: 0.125rem;">{{ Number(timeData.seconds) < 10 ? `0${timeData.seconds}` : timeData.seconds }}</em>
                </template>
              </van-count-down>
            </div>
            <div class="time" v-if="isBuy && isBegTime">
              <span class="text">距开始还剩：</span>
              <van-count-down :time="fun.getTimeDifference(beginTime)" class="bottom_time" format="DD天HH时mm分ss秒" @finish="timeEnd">
                <template #default="timeData">
                  <em style="padding: 0.25rem 0.125rem; border-radius: 0.125rem;">{{ timeData.days < 10 ? `0${timeData.days}` : timeData.days }}</em
                  ><span style="color: #fff;">天</span>
              ...
              <div class="title-box">
                ...
                <h2>
                  <van-col :span="24" id="price" v-if="!isGoodsLove">
                    <div class="show_price_box" v-if="goodsInfo.goods_show && goodsInfo.goods_show.goods_detail_style == 1">
                    ...
                    <template v-if="!goodsInfo.goods_show || goodsInfo.goods_show.goods_detail_style != 1">
          ...
          <!-- 一口价入口 -->
          <div class="optional" @click="toFixedPrice()" v-if="hasActivity">
            <span class="three-count">{{ pack_price }}元任选{{ goods_count }}件</span>
            <span class="go-group">去凑单<i class="iconfont icon-advertise-next icon-arrow"></i></span>
          </div>
          ...
          <div class="act-box">
            ...
            <!-- 自购省 -->
            <ul class="act" @click="gotohuiyuan" v-if="goods_info.selfbuy_discount" style="height: auto; align-items: center;">
              <li class="act1" style="color: #999;">自购省</li>
              <li class="act2" style="height: auto;">
                <div v-if="goods_info.selfbuy_discount.this_level_discount_price.show == 1">
                  {{ goods_info.selfbuy_discount.this_level_discount_price.content }}
                </div>
                <div v-if="goods_info.selfbuy_discount.next_level_discount_price.show == 1">
                  {{ goods_info.selfbuy_discount.next_level_discount_price.content }}
                </div>
              </li>
          ...
          <!-- 供应商入口 -->
          <div class="supplier-box" @click="gotoSupplier()" v-if="!this.fun.isTextEmpty(supplierData) && supplierData.is_open == 1">
            <div style="flex: 1; display: flex;">
              ...
              <ul class="user-name">
                <li>
                  {{ this.fun.isTextEmpty(supplierData.store_name) ? supplierData.username : supplierData.store_name }}
                </li>
                <li>
      ...
      <van-popup v-model="popupSpecs2" :style="{ overflow: 'visible' }" position="bottom" class="mint-popup-4" closeOnClickModal="true">
        <div class="specifications">
          ...
          <!--  -->
          <van-button size="small" id="btsmall" type="danger" @click="buyNowNew" v-show="popStock >= 1">确认</van-button>
          <van-button type="danger" color="#999999" size="small" disabled id="btsmall" v-show="popStock <= 0">
            确认
          </van-button>
        </div>
      ...
      <template v-if="showComment">
        <van-popup v-model="showComment" duration="0" position="right" style="height: 100%; width: 100%;">
      ...
      <van-popup v-model="showParameter" position="bottom" :style="{ height: '60%' }">
        ...
        <div style="clear: both; margin-top: 2.8125rem;" class="pcStyle_ydC">
          <ul class="parameterClass">
            ...
            <p class="notext" v-if="fun.isTextEmpty(goodsInfo.has_many_params)">(＞﹏＜)<br />暂无该商品参数，请联系客服 ~</p>
          </ul>
        </div>
      </van-popup>
      <div v-if="isCup">
      ...
      <div id="foot">
        <div class="addfav" @click="gotoSupplier" v-if="isCup">
          <template>
            <ul>
              <li>
        ...
        <div :class="{ cart: true, cell: true, nocar: !isGoods || !isAddCart }" @click="addCart" v-if="isCup && !isBegTime">
          加入购物车
        </div>
      ...
      <div style="height: 0;" v-if="fun.isTextEmpty(goodsInfo.show_push) ? false : true"></div>

      <div style="height: 3.125rem;" v-if="fun.isTextEmpty(goodsInfo.show_push)"></div>
