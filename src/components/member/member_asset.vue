  ...
  >
    <div class="extension_plate" v-if="Params.assetstyle == 1">
      ...
      <ul class="box" id="template02">
        <template
          v-if="Order.part && Order.part.length > 0 && notShow.indexOf(item.url) < 0"
        ...
        <li v-for="(item, i) in Order.more" :key="i" v-show="Order.more && Order.more.length > 0"
            @click="gotoUrl(item)">
            ...
            >
              <div class="list1" @click="pluginGoto(item)" :key="index">
                <i
                  ...
                  class="iconfont"
                  :class="item.class"
                  style="margin-right: 0.375rem; font-size: 1.625rem; color: #f15353;"
                ></i>
...
<style lang="scss" rel="stylesheet/scss" scoped>
  ...
  #template02 {
    ...
    .icon-fontclass-xingpinle::before {
      background: url('https://www.yunzmall.com/min_img/member/member_a(133).png') no-repeat center;
      background-size: 1.75rem;
      width: 1.75rem;
      height: 1.75rem;
    }

    .icon-fontclass-xingpinle::before::before {
      content: '';
    }
