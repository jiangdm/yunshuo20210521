<template>
  <div :class="className">
    ...
    <!--文章静态-->
    <div>
      <div id="article" :style="{ backgroundColor: datas.article_bg }">
        ...
        <template v-if="Number(datas.get_number) > 0 && datas.list && (datas.get_style == 1 || datas.get_style == 3)">
          <div class="article-list" v-for="(item, index) in list" :key="index" @click.stop.prevent="gotoArticle(item)">
            <!-- 样式一图片 -->
...
export default {
  ...
  methods: {
    ...
    getNetData(data) {
      ...
      if(data) {
        this.noNotice = false;
        that.total_page = data.list.last_page;
      ...
      $http.get("plugin.article.api.article.get-articles", json, "获取中").then(
        response => {
          if (response.result == 1) {
            ...
            if(response.data.articles.data.length > that.datas.get_number) {
              response.data.articles.data.splice(that.datas.get_number, response.data.articles.data.length-this.datas.get_number);
              that.isLoadMore = false;
              this.allLoaded = true;
            }

            that.list = response.data.articles.data;
    ...
    getMoreData(page) {
      ...
      if (this.page >= this.total_page || this.list.length > this.datas.get_number) {
        ...
        let json = { category_id: this.class_id, page: this.page, pageSize: 12 };
        $http
          ...
          .then( (response)=> {
            if (response.result === 1) {
              ...
              that.list = that.list.concat(response.data.articles.data);
