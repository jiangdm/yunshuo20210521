  ...
  <div class="component-wrapper" :style="wrapper_style">
    <template v-if="list && list.length > 0">
      <div style="position: relative;" v-for="(item, index) in list" :key="index">
        ...
        <template v-if="item.hotArea && item.hotArea.length > 0">
          <div
            class="hotarea-rnd"
          ...
          >
            <div style="position: absolute; width: 100%; height: 100%; top: 0; left: 0;" v-if="area.is_minApp == 2" @click.stop v-html="area.html"></div>
          </div>
        </template>
      </div>
    </template>
...
export default {
  ...
  methods: {
    toUrl(item) {
      if (item.is_minApp == 1 && item.h5_link && this.datas.choose_style == 2) {
