<template>
  <div class="U000026-template1" style="position: relative;"
       :class="className">
    <!-- style -->
    <div v-html="css"></div>
    ...
    <div class="shortvideo" v-if="datas.preview_color">
      <div class="list" v-if="Number(datas.get_number) > 0 && datas.get_style == 1">
        <c-video :id="id" :showLoading="showLoading" :isLoadMore="isLoadMore" :isDiy="true" @loadMore="loadMore"
                 @getMore="getMoreData" :list='list'></c-video>
      </div>

      <div class="list" v-if="datas.list.length > 0 && datas.get_style == 2">
        <c-video :id="id" :showLoading="showLoading" :isDiy="true" @loadMore="loadMore" :list='datas.list'></c-video>
      </div>
    </div>
  </div>
</template>
...
export default {
  props: ["page_id", "id", "datas", "index", "component_key", "tabcontrol"],
  data() {
    return {
      emptyImage,
      list: [],
  ...
  methods: {
    ...
    getMoreData() {
      ...
      if (this.page >= this.total_page || this.list.length > this.datas.get_number) {
        ...
        $http
          .get("home-page.get-decorate-page", json, "loading")
          .then((response) => {
            this.isLoadMore = true;
