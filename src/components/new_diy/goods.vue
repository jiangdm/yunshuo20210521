  ...
  <div :class="className" :style="{ backgroundColor: datas.preview_color }">
    ...
    <div class="u_goodslist" v-if="datas.preview_color">
      <div class="u_goodslist_two" v-if="datas.sort_style.choose_icon == 1">
        ...
        <!-- 大图失效的 两种样式 是一样-->
        <div
          v-if="(datas.list.data && datas.list.data.length <= 0) || datas.list.length <= 0"
        ...
        >
          <div
            ...
            :class="[datas.disable_good == 1 ? 'failure_cover' : '']"
          >
          ...
          >
            <!-- 限时抢购 -->
            <img
              src="../../assets/images/new_diy/xianshiqianggou@2x.png"
              v-if="datas.good_tag.choose_icon == 5"
              alt=""
            />
            <!-- 包邮 -->
          ...
          <div class="twoChild_bottom">
            ...
            <div class="Child_other">
              <div class="oneChild_sales" style="color: #fff;" v-if="datas.content_list.includes('7')">
                销量 {{ item.sales }}
              </div>
              <div
                class="oneChild_activity"
                style="color: #fff;"
                v-if="!datas.content_list.includes('5') && datas.content_list.includes('6') && item.point != 0"
              >
              ...
              <div
                ...
                style="color: #fff;"
                v-if="
                  datas.content_list.includes('5') &&
                    item.award == 1 &&
                    (!item.vip_level_status || (item.vip_level_status && item.vip_level_status.status == 0))
                "
              >
                赠送{{ item.award_proportion }}%{{ item.love_name || '爱心值' }}
              </div>
            </div>
            <div class="Child_member_price" v-if="datas.content_list.includes('4') && !item.point_goods">
              会员价
              {{
            ...
            <div class="Child_price">
              <div class="Child_now_price" :style="{ width: datas.button_type == 0 ? '100%' : '70%' }">
                <span
                  ...
                  v-if="
                    (datas.content_list.includes('3') && !datas.content_list.includes('8')) ||
                      (datas.content_list.includes('8') && item.coupon && item.coupon.coupon_method === 0)
                  "
                  >
                  <template v-if="!item.point_goods">
                    <em v-if="datas.show_price_text">{{ price_text }}</em><em>{{ $i18n.t('money') }}</em>
                  </template>
                  {{item.point_goods ? item.point_goods.min_point
                  :(item.vip_level_status && item.vip_level_status.status == 1
                      ? item.vip_level_status.word
                      : item.pricenow)
                  }}
                ...
                <span v-if="datas.content_list.includes('8') && item.coupon && item.coupon.coupon_method !== 0"
                  ...
                  {{ item.coupon.deduct_price }}</span
                >
                <em
                  class="oneChild_original_price"
                  v-if="datas.content_list.includes('2') && item.priceold != 0 && item.priceold != item.pricenow && !item.point_goods"
            ...
            <div class="show_price_box" v-if="!fun.isTextEmpty(item.goods_show)">
              <div
                ...
                v-for="(items, i) in item.goods_show"
                :key="i"
              >
        ...
        >
          ...
          <div class="threeChild_b">
            ...
            <div class="Child_price">
              ...
              >
                <template v-if="!item.point_goods">
                  <em v-if="datas.show_price_text">{{ price_text.substring(0, 1) }}</em><em>{{ $i18n.t('money') }}</em>
                </template>
                {{item.point_goods ? item.point_goods.min_point
                  :(item.vip_level_status && item.vip_level_status.status == 1
                    ? item.vip_level_status.word
              ...
              <div
                class="Child_now_price"
                v-if="!datas.content_list.includes('3') && !datas.content_list.includes('8')"
              ></div>
              <div class="Child_buyBtn">
                <div class="buyBtn" :class="`buyBtn_${datas.button_type}`">
                  <template v-if="datas.button_type == 1 || datas.button_type == 2">{{ datas.button_title }}</template>
          ...
          <div class="oneChild_right">
            ...
            <div class="oneChild_r_flexbottom">
              ...
              <div class="Child_price">
                ...
                <div class="Child_buyBtn">
                  <div class="buyBtn" :class="`buyBtn_${datas.button_type}`">
                    <template v-if="datas.button_type == 1 || datas.button_type == 2">{{
                      datas.button_title
                    }}</template>
                    <i class="iconfont icon-gaiicon-" v-if="datas.button_type == 3 || datas.button_type == 4"></i>
                    <i class="iconfont icon-life-game-plus" v-if="datas.button_type == 5 || datas.button_type == 6"></i>
          ...
          <div class="threeChild_b">
            ...
            <div class="Child_price">
              <div
                class="Child_now_price"
                :style="{
                  width: datas.button_type == 0 || datas.button_type == 1 || datas.button_type == 2 ? '100%' : '80%',
                  fontSize: item.vip_level_status && item.vip_level_status.status == 1 ? '12px' : ''
                }"
                v-if="
                  (datas.content_list.includes('3') && !datas.content_list.includes('8')) ||
                  ...
                  <div class="Child_buyBtn">
                    <div class="buyBtn" :class="`buyBtn_${datas.button_type}`">
                      ...
                      <i
                        ...
                        v-if="datas.button_type == 5 || datas.button_type == 6"
                      ></i>
                    </div>
                  </div>
                </div>
            ...
            <div class="u_goodslist_three_child" @click.stop.prevent="gotoDetail(item)">
              ...
              <div class="threeChild_top">
                ...
                >
                  ...
                  <!-- 推荐 -->
                  <img
                    src="../../assets/images/new_diy/recommend@2x.png"
                    v-if="datas.good_tag.choose_icon == 1"
                    alt=""
                  />
                  <!-- 自定义 -->
                  <img :src="datas.good_tag.image" style="width: 100%;" v-if="datas.good_tag.choose_icon == 7" alt="" />
...
const css = function() {
  if (this.datas.preview_color) {
    ...
    return `
        ...
        .component-${this.id} .u_goodslist .Child_buyBtn .buyBtn_2, .component-${
  this.id
} .u_goodslist .Child_buyBtn .buyBtn_4, .component-${this.id} .u_goodslist .Child_buyBtn .buyBtn_6 {
        ...
        .component-${this.id} .u_goodslist_two_child, .component-${this.id} .u_goodslist_three_child, .component-${
  this.id
} .u_goodslist_four_child, .component-${this.id} .u_goodslist_one_child {
        ...
        .component-${this.id} .u_goodslist_two_child.big, .component-${this.id} .u_goodslist_one_child  {
            ...
            box-shadow: ${show_shadow ? '1' : '0'}px ${show_shadow ? '1' : '0'}px ${
  show_shadow ? '5' : '0'
}px ${border_color};
        }
...
export default {
    ...
    isBottom: function(newVal, oldVal) {
      if (this.tabcontrol) {
        ...
        if (newVal && this.isLast && this.datas.sort_style.choose_icon != 5 && this.isLoadMore) {
          ...
          if (this.alreadyHandleArray.slice(12 * this.arrIndex).length > 0) {
            this.datas.list.data = this.datas.list.data.concat(
              this.alreadyHandleArray.slice(12 * this.arrIndex, 12 * (this.arrIndex + 1))
            ); //数组拼接
  ...
  mounted() {
    ...
    if (this.datas.list.data) {
      ...
      this.datas.list.data = this.datas.list.data.slice(0, 12);
    } else {
  ...
  methods: {
    ...
    copyGoodsUrl(id) {
      let _link =
        this.fun.getSiteRoot() + '/addons/yun_shop/' + '?menu#' + '/goods/' + id + '?i=' + this.fun.getKeyByI();
    ...
    gotoDetail(good) {
      ...
      this.$router.push(this.fun.getUrl(url, { id: good.id }));
    },
    orderScroll() {
    ...
    // 获取更多数据
    getMoreData() {
      if (this.alreadyHandleArray.slice(12 * this.arrIndex).length >= 12) {
        this.datas.list.data = this.datas.list.data.concat(
          this.alreadyHandleArray.slice(12 * this.arrIndex, 12 * (this.arrIndex + 1))
        ); //数组拼接
        this.arrIndex = this.arrIndex + 1;
