  ...
  <div :class="className" :style="{'backgroundColor':datas.preview_color}">
    ...
    <div class="u_goodslist" v-if="datas.preview_color">
      ...
      <div class="u_goodslist_two" v-if="datas.sort_style.choose_icon == 1">
        <!-- 大图样式 -->
        <!-- 大图失效的 两种样式 是一样-->
        <div v-if="datas.list.length <= 0" style="width: 100%; height: 150px; text-align: center;">
          <img
        ...
        <div class="u_goodslist_two_child big u_goodslist_two_child_only" v-for="(item, index) in datas.list" :key="index" v-if="item && item.status != 2 || datas.disable_good != 0" @click.stop.prevent="gotoDetail(item)">
          <div class="Failure" v-if="datas.disable_good != 0 && item.status == 2" :class="[datas.disable_good == 1?'failure_cover':'']">
            <div class="Failure_text">已结束</div>
          </div>
          <img :src="item.goods_thumb || emptyImage" alt="" class="goodsImg">
          ...
          <div class="twoChild_bottom">
            <div class="twoChild_top" v-if="checkTime(item.start_time)">
              <i class="iconfont icon-ht_qd_daojishi" style="margin: 0 5px;"></i>
              <van-count-down :time="(item.start_time *1000) - Date.parse(new Date())">
            ...
            <div class="twoChild_line">
              <div class="posRed" style="width: 100%;">满{{item.member_num}}人成团</div>
            </div>
            ...
            <div class="Child_price">
              <div class="Child_now_price" :style="{width: datas.button_type == 0 ? '100%' : '70%'}">
                <span v-if="datas.content_list.includes('3')"><em>{{$i18n.t('money')}}</em> {{item.group_price}}</span>
                <em class="oneChild_original_price" v-if="datas.content_list.includes('2')">{{$i18n.t('money')}}{{item.goods_price}}</em></div>
      ...
      <div class="u_goodslist_three" v-if="datas.sort_style.choose_icon == 2">
        ...
        <!-- 双排样式 -->
        <div class="u_goodslist_three_child " v-for="(item, index) in datas.list" :key="index" v-if="item && item.status != 2 || datas.disable_good != 0" @click.stop.prevent="gotoDetail(item)">
          <div class="Failure" v-if="datas.disable_good != 0 && item.status == 2" :class="[datas.disable_good == 1?'failure_cover':'']">
            <div class="Failure_text">已结束</div>
          </div>
          <div class="threeChild_top">
            ...
            <img :src="item.goods_thumb || emptyImage" alt="" class="goodsImg">
            <div class="twoChild_topPos" v-if="checkTime(item.start_time)">
              <i class="iconfont icon-ht_qd_daojishi" style="margin: 0 5px;"></i>
              <van-count-down :time="(item.start_time *1000) - Date.parse(new Date())">
                <template #default="timeData">
                  <span class="time">{{ timeData.days }}</span>
          ...
          <div class="threeChild_b">
            ...
            <div class="Child_price">
              <div class="Child_now_price" :style="{width: datas.button_type == 1 || datas.button_type == 2 ? '50%' : '80%'}" v-if="datas.content_list.includes('3')"><em>{{$i18n.t('money')}}</em> {{item.group_price }}</div>
            ...
            <div class="oneChild_original_price" v-if="datas.content_list.includes('2')">{{$i18n.t('money')}}{{item.goods_price}}</div>
          </div>
        </div>
      </div>
      ...
      <div class="u_goodslist_one" v-if="datas.sort_style.choose_icon == 3">
        ...
        <!-- 横排样式 -->
        <div class="u_goodslist_one_child " v-for="(item, index) in datas.list" :key="index" v-if="item && item.status != 2 || datas.disable_good != 0" @click.stop.prevent="gotoDetail(item)">
          <div class="Failure" v-if="datas.disable_good != 0 && item.status == 2" :class="[datas.disable_good == 1?'failure_cover':'']">
            <div class="Failure_text">已结束</div>
          </div>
          <div class="oneChild_left">
            ...
            <img :src="item.goods_thumb || emptyImage" alt="">
            <div class="twoChild_topPos" v-if="checkTime(item.start_time)">
              <i class="iconfont icon-ht_qd_daojishi" style="font-size: 12px;"></i>
              <van-count-down :time="(item.start_time *1000) - Date.parse(new Date())">
                <template #default="timeData">
                  <span class="time">{{ timeData.days }}</span>
                  <span class="colon">天</span>
      ...
      <div class="u_goodslist_four" v-if="datas.sort_style.choose_icon == 4">
        ...
        <!-- 三排排序样式 -->
        <div class="u_goodslist_four_child " v-for="(item, index) in datas.list" :key="index" v-if="item && item.status != 2 || datas.disable_good != 0" @click.stop.prevent="gotoDetail(item)">
          <div class="Failure" v-if="datas.disable_good != 0 && item.status == 2" :class="[datas.disable_good == 1?'failure_cover':'']">
            <div class="Failure_text">已结束</div>
          </div>
          <div class="threeChild_top">
            ...
            <img :src="item.goods_thumb || emptyImage" alt="" class="goodsImg">
            <div class="twoChild_topPosThr" v-if="checkTime(item.start_time)">
              <i class="iconfont icon-ht_qd_daojishi" style="font-size: 12px;"></i>
              <van-count-down :time="(item.start_time *1000) - Date.parse(new Date())">
                <template #default="timeData">
                  <span class="time">{{ timeData.days }}</span>
                  <span class="colon">天</span>
                  <span class="time" style="width: 15px;">{{ timeData.hours }}</span>
                  <span class="colon" style="width: 15px;">:</span>
                  <span class="time">{{ timeData.minutes }}</span>
                  <span class="colon" style="width: 15px;">:</span>
                  <span class="time">{{ timeData.seconds }}</span>
                </template>
          ...
          <div class="threeChild_b">
            ...
            <div class="Child_other">
              ...
              <div class="twoChild_line">
                <div class="posRed" style="width: 100%; border-radius: 10px;">满{{item.member_num}}人成团</div>
              </div>
            </div>
            <!-- <div class="Child_member_price" v-if="datas.content_list.includes('4')">会员价 500.00</div> -->
            <div class="Child_price">
              <div class="Child_now_price" :style="{width: datas.button_type == 0 || datas.button_type == 1 || datas.button_type == 2 ? '100%' : '80%'}" v-if="datas.content_list.includes('3')"><em>{{$i18n.t('money')}}</em> {{item.group_price }}</div>
              <div class="Child_now_price" v-if="!datas.content_list.includes('3')"></div>
      ...
      <div class="u_goodslist_four  u_goodslist_five" v-if="datas.sort_style.choose_icon == 5">
        ...
        <!-- 三排滑动样式 -->
        <div class="u_goodslist_four_child u_goodslist_five_child" v-for="(item, index) in datas.list" :key="index" v-if="item && item.status != 2 || datas.disable_good != 0" @click.stop.prevent="gotoDetail(item)">
          <div class="Failure" v-if="datas.disable_good != 0 && item.status == 2" :class="[datas.disable_good == 1?'failure_cover':'']">
            <div class="Failure_text">已结束</div>
          </div>
          <div class="threeChild_top">
      ...
      <div class="u_goodslist_six" v-if="datas.sort_style.choose_icon == 6">
        ...
        <!-- 一大两小 -->
        <template v-for="(item, index) in datas.list" v-if="item && item.status != 2 || datas.disable_good != 0">
            ...
            <div class="u_goodslist_three_child" @click.stop.prevent="gotoDetail(item)">
              ...
              <div class="threeChild_top">
                <div class="good_Attributes" :style="{width: datas.good_tag.choose_icon ==7? '35px':'',height: datas.good_tag.choose_icon ==7? 'auto':''}">
                  ...
                  <img src="../../assets/images/new_diy/recommend@2x.png" v-if="datas.good_tag.choose_icon ==1" alt="">
                  <!-- 自定义 -->
                  <img :src="datas.good_tag.image || emptyImage" style="width: 100%;" v-if="datas.good_tag.choose_icon ==7" alt="">
                </div>
                <img :src="item.goods_thumb || emptyImage" alt="" class="goodsImg">
                <div class="twoChild_topPos" v-if="checkTime(item.start_time)">
                  <i class="iconfont icon-ht_qd_daojishi" style="margin: 0 5px;"></i>
